    ...
    # =============================================================================
        ...
        # KIỂM TRA ĐIỀU KIỆN 2: Nếu ĐÃ số hóa xong file SBD -> Màn hình tác nghiệp sản xuất
            ...
            if isinstance(sbd_data_store, dict) and sbd_data_store:
                ...
                with btn_col2:
                    trigger_consumption = st.button("🤖 2. KÍCH HOẠT NHẢY SỐ ĐỊNH MỨC VÀ ĐỐI CHIẾU CAD", type="secondary", use_container_width=True, key="c2_consumption_btn")

                # Bẫy kích hoạt thuật toán sơ đồ mẫu ban đầu
                if trigger_auto_cutting:
                    mock_results = []
                    for i, sz in enumerate(active_sizes):
                        mock_results.append({
                            "Sơ đồ / Trạng thái": f"c{str(i+1).zfill(2)}",
                            "Ratios": {s: (1 if s == sz else 0) for s in active_sizes},
                            "Số lớp": 50, "Số bàn": 1, "Số sp/SĐ": 1
                        })
                    mock_results.append({
                        "Sơ đồ / Trạng thái": "Balance",
                        "Ratios": {s: 0 for s in active_sizes},
                        "Số lớp": 0, "Số bàn": 0, "Số sp/SĐ": 0
                    })
                    st.session_state["auto_cutting_results"] = mock_results
                    st.success("✅ Đã tính toán tỷ lệ sơ đồ gốc! Hãy dán chiều dài CAD và ấn tiếp nút số 2.")
                ...
                if st.session_state.get("auto_cutting_results") is not None:
                    ...
                    for col_name in active_sizes:
                        ...
                        if "giàng" in col_clean.lower() or any(char in col_clean.lower() for char in ["x", "-", "/"]):
                            ...
                            if len(parts_clean) >= 2:
                                ...
                                giang_val = parts_clean[1]; size_val = parts_clean[0]
                            elif len(parts_clean) == 1:
                                size_val = parts_clean[0]; giang_val = ""
                            else:
                                size_val = col_clean; giang_val = ""
                        else:
                            size_val = col_clean; giang_val = ""
                    ...
                    # --- XUẤT FILE EXCEL CHUẨN THƯƠNG MẠI ---
                    try:
                        buffer = io.BytesIO()
                    ...
                    # 🎯 DỰNG GIAO DIỆN WEB 3 TẦNG ĐỒNG BỘ
                    ...
                    for item in parsed_size_columns:
                        ...
                        if is_don_khong_giang:
                            web_multi_cols.append((f"SIZE TRẦN", f"{item['size_num']}", f"{po_qty_val}"))
                        else:
