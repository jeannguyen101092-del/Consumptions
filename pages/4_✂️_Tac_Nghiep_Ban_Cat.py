...
for i, sz in enumerate(active_sizes):
    ...
    # Đồng bộ lưu song song cả vào key tên size thật
    giang_top_row[str(sz)] = "0"
    size_top_row[str(sz)] = str(sz)
    sl_top_row[str(sz)] = safe_int_final(real_size_breakdown.get(sz, 0))
...
# --- 2. KHỞI TẠO HOẶC KHÔI PHỤC SNAPSHOT BỘ NHỚ ĐỆM Ô LƯỚI ---
...
if is_snapshot_valid:
    ...
    for row in filtered_snapshot:
        ...
        for c_idx, sz in enumerate(active_sizes):
            val_cell = row.get(f"CỠ {c_idx+1}", row.get(str(sz), "0"))
            item_dict[f"CỠ {c_idx+1}"] = str(val_cell).strip()
            item_dict[str(sz)] = str(val_cell).strip()
...
# =============================================================================
# TẦNG 3 - ĐOẠN 6b: PHẦN 2 - RENDERING DATA EDITOR & ĐỒNG BỘ SỐ LIỆU TỔNG SL REALTIME V41
# =============================================================================
...
def callback_sync_on_the_fly_final():
    if "table_manual_data_editor_final_clean_v1" in st.session_state:
        ...
        # 1. Đồng bộ hành động gõ/sửa/paste dữ liệu từ người dùng vào bộ nhớ đệm snapshot trung tâm
                    ...
                    for col_header, new_val in change_dict.items():
                        if str(col_header).startswith("CỠ "):
                            try:
                                ...
                                # Ghi nhận giá trị chuỗi văn bản vào cả 2 loại key (Key hiển thị và Key tên size thật)
        ...
        # 2. Tự động dò dòng SẢN LƯỢNG để cộng dồn tổng đơn hàng PO thời gian thực
        if len(current_snapshot) > 2:
            ...
            if sl_row_idx is not None:
                ...
                for c_idx in range(len(active_sizes_cb)):
                    val_cell = current_snapshot[sl_row_idx].get(f"CỠ {c_idx+1}", 0)
                    calc_total += safe_int_final(val_cell)
...
# Đọc dữ liệu snapshot thời gian thực chuẩn bị render hiển thị
...
# 🎯 TOÁN HỌC KHỬ CHUỖI TEXT CHUYÊN DỤNG: Ép về dạng số nguyên sạch để nhân (Tỷ lệ phối * Sơ lớp * Số bàn)
for idx, row in df_editor_top_render.iterrows():
    ...
    if row_name not in ["GIÀNG", "SIZE"]:
        ...
        for i in range(len(active_sizes)):
            # Lọc bóc tách số nguyên thuần túy từ ô lưới gõ tay
        ...
        if row_name != "SẢN LƯỢNG":
            ...
            # Thực hiện phép nhân số lượng thực tế hiển thị ngay lên cột Tổng SL
...
# --- NÚT BẤM KÍCH HOẠT VÀ ĐÓNG GÓI SỐ SẠCH TRUYỀN ĐI ---
...
if st.button("🚀 KÍCH HOẠT THUẬT TOÁN TỰ ĐỘNG CHIA SƠ ĐỒ VÀ VẾT ĐƠN", type="primary", use_container_width=True, key="trigger_python_engine_solve_btn"):
    ...
    for idx, row in df_editor_top_render.iterrows():
        ...
        # 🎯 CHỐNG NGHẼN LUỒNG PHÉP TÍNH: Ép chuyển đổi toàn bộ mảng dữ liệu chữ gõ tay về số nguyên gốc 
        # Để đảm bảo Engine toán học 7a nhận diện được lượng hàng dán và gõ tay
        if row_name not in ["GIÀNG", "SIZE"]:
            for i in range(len(active_sizes)):
                sz_key = str(active_sizes[i])
                val_int = safe_int_final(row_dict.get(f"CỠ {i+1}", 0))
                
                # Ghi đè số nguyên sạch vào payload snapshot truyền đi
                row_dict[f"CỠ {i+1}"] = val_int
                row_dict[sz_key] = val_int
        else:
            # Hàng GIÀNG và SIZE thì giữ chữ văn bản text chuỗi
            if idx < len(snapshot_current_live):
                orig_row = snapshot_current_live[idx]
                for sz in active_sizes:
                    sz_str = str(sz)
                    if sz_str in orig_row:
                        row_dict[sz_str] = str(orig_row[sz_str]).strip()
                        
        list_rows_clean.append(row_dict)
        
    st.session_state["session_editor_snapshot"] = list_rows_clean
    st.session_state["c2_normal_cut_btn"] = True
    st.success("🤖 Đang truyền ma trận số liệu và gọi Engine Python khấu trừ điều độ...")
    st.rerun()


import math
# =============================================================================
# TẦNG 3 - ĐOẠN 7a: ENGINE TOÁN HỌC ĐIỀU ĐỘ NGƯỢC THÔNG SUỐT LINK BIẾN V41
# =============================================================================
...
if st.session_state.get("c2_normal_cut_btn", False):
    ...
    # 🎯 1. SỬA ĐỒNG BỘ KHẤU TRỪ: Đọc sản lượng đầu vào bám sát theo nhãn cột CỠ X ảo đã gõ tay
    if snapshot_current:
        for row in snapshot_current:
            if str(row.get("BÀN CẮT / TÊN SƠ ĐỒ", "")).upper().strip() == "SẢN LƯỢNG":
                for idx_sz, sz in enumerate(active_sizes):
                    # Bóc tách số nguyên an toàn từ ô lưới ảo hiển thị của hàng sản lượng
                    current_order_balances[sz] = safe_int_final(row.get(f"CỠ {idx_sz+1}", 0))
                break
                
    total_sum_po_qty = sum(current_order_balances.values())
    
    # Nạp 3 hàng thông tin mốc gốc vào danh sách báo cáo đầu ra
    for row in snapshot_current:
        if str(row.get("BÀN CẮT / TÊN SƠ ĐỒ", "")).upper().strip() in ["GIÀNG", "SIZE", "SẢN LƯỢNG"]:
            final_snapshot_rows.append(row)

    # 🎯 2. SỬA ĐỒNG BỘ KHẤU TRỪ DÒNG PHỐI TAY (PILOT / C01): Trừ lùi đơn hàng dựa theo nhãn ô hiển thị
    for row in snapshot_current:
        ...
        if row_name not in ["GIÀNG", "SIZE", "SẢN LƯỢNG"] and ("PILOT" in row_name or "SS" in row_name or "C" in row_name):
            ...
            if layers > 0:
                r_dict = {}
                for idx_sz, sz in enumerate(active_sizes):
                    # Đọc tỷ lệ phối tay từ nhãn ô hiển thị ảo
                ...
                # Giữ nguyên bản ghi sơ đồ phối tay lên báo cáo, chừa trống dài sơ đồ 0.0
                final_snapshot_rows.append(row)

    # 3. Thu thập định mức tài liệu và Chiều dài tối đa bàn vải để làm mốc rải vải cuốn chiếu
    consumption_in_yards = st.session_state.get("consumption_input_key_unique", 1.140)
    dm_met = round(consumption_in_yards * 0.9144, 4)  # Đổi Yards sang Mét bàn vải
    
    max_table_len_val = st.session_state.get("max_table_length_key_unique", 12.0)
    max_target_layers = 60  # Số lớp vải rải kịch khung bàn vải quy định

    # Tính tổng số sản phẩm cực đại trên sơ đồ chính đầu tiên dựa vào chiều dài bàn vải tối đa
    ...
    # Vòng lặp chia tỷ lệ phân bổ động cuốn chiếu nhỏ dần
    while sum(current_order_balances.values()) > 0 and marker_counter <= max_safety_loops:
        ...
        # Dựng dòng sơ đồ tự động tính toán ra cột ảo hiển thị báo cáo
        item_auto = {
            "BÀN CẮT / TÊN SƠ ĐỒ": s_marker_name, "SƠ LỚP": row_layers, "SỐ BÀN": 1,
            "DÀI SƠ ĐỒ": 0.0, # Chừa trống tuyệt đối độ dài mốc 0.0 cho xưởng tự dán từ CAD phần mềm
            "TỔNG SẢN LƯỢNG": row_ratios_total * row_layers
        }
        for idx_sz, sz in enumerate(active_sizes):
            item_auto[f"CỠ {idx_sz+1}"] = r_dict.get(sz, 0)
            item_auto[str(sz)] = r_dict.get(sz, 0)
    ...
    # 4. Giai đoạn cuối đơn: Chèn dòng vét sạch toàn bộ số lượng lẻ còn sót lại của khúc vải
    ...
    # Đẩy mảng số liệu phẳng hoàn chỉnh lên bộ nhớ đệm trung tâm kết xuất báo cáo
...
import math
# =============================================================================
# TẦNG 3 - ĐOẠN 7a: KHỐI ENGINE TOÁN HỌC TÁC NGHIỆP NGƯỢC CHUẨN XÁC V37
# =============================================================================
...
if st.session_state.get("c2_normal_cut_btn", False):
    ...
    while sum(current_order_balances.values()) > 0 and marker_counter <= max_safety_loops:
        ...
        # Quy tắc giảm dần dung lượng tỷ lệ phối sơ đồ cho vòng tiếp theo để vét đơn mượt mà
        garments_per_marker = max(2, garments_per_marker - 1)
        marker_counter += 1
...
# =============================================================================
# TẦNG 3 - ĐOẠN 7b (PHẦN 1): TÁI CẤU TRÚC PHIẾU BÁO CÁO FIX CHUẨN GIÀNG QUẦN SHORT V37
# =============================================================================
...
# Đọc an toàn thông tin tổng quát từ session_state widget
...
# ĐỒNG BỘ DANH SÁCH SIZE TOÀN CỤC THỰC TẾ
active_sizes = st.session_state.get("active_sizes_global", [])
if not active_sizes:
    active_sizes = ["26", "27", "28", "29", "30", "31", "32", "33", "34"]

# Đọc mảng ma trận từ bộ nhớ snapshot phiên bản mới nhất
snapshot_current = st.session_state.get("session_editor_snapshot", [])
...
# 🎯 BẢN VÁ MẤU CHỐT: Tách chuỗi hình học động cho bảng báo cáo, tự ép giàng quần short về 0
for col_name in active_sizes:
    ...
    if len(parts) >= 2:
        ...
        s_val = str(parts[0]).strip()
        g_val = "0"  # Giải quyết triệt để bài toán đồ short, ko bịa số 30
...
for r_idx, row_data in enumerate(production_rows):
    ...
    for c_idx, sz in enumerate(active_sizes):
        # Trích xuất tỷ lệ phối từ ô lưới ảo
        r_val = safe_int_final(row_data.get(f"CỠ {c_idx+1}", 0))
        ratios_sum += r_val
        row_ratios_list.append(r_val)
    
    # Tính toán định mức sơ đồ thực tế
    ...
    # Đóng gói hàng tỷ lệ sản xuất bàn vải
    ...
    # Chèn dòng snapshot lượng tồn dư "CÒN LẠI" khấu trừ lũy tiến xuống dưới
...
# =============================================================================
# ĐOẠN CODE BỔ SUNG: NÚT XÓA SẠCH DỮ LIỆU ĐỂ LẬP PHIẾU MỚI - ĐÃ TỐI ƯU GÕ TAY V37
# =============================================================================
st.markdown("<br>", unsafe_allow_html=True)
...
with clear_col2:
    if st.button("🗑️ XÓA TOÀN BỘ DỮ LIỆU TÁC NGHIỆP HIỆN TẠI", use_container_width=True, type="secondary", key="clear_all_planning_data_final_btn"):
        # 1. Kích hoạt cờ xóa dữ liệu bảo vệ hệ thống
        st.session_state["planning_cleared"] = True
        
        # 2. Reset toàn bộ bộ nhớ đệm lưu trữ gõ tay và dán chữ Excel
        st.session_state["manual_parsed_breakdown"] = {}
        st.session_state["current_po_target_val"] = 0
        st.session_state["active_sizes_global"] = ["26", "27", "28", "29", "30", "31", "32", "33", "34"]
        
        # Đánh sập toàn bộ ô lưới cũ để ép Bảng 1 dựng lại từ đầu mốc số 0 sạch sẽ
        st.session_state["session_editor_snapshot"] = None
        
        # Giải phóng các bộ nhớ đệm thuật toán rải vải
        ...
        # 3. Tắt cờ phòng vệ sẵn sàng đón đơn hàng dán tiếp theo
        st.session_state["planning_cleared"] = False
        
        st.success("🎉 Đã dọn dẹp sạch kho dữ liệu tác nghiệp cũ thành công!")
        st.rerun()  # Làm mới trang ngay lập tức để làm tươi lại ma trận ô lưới mốc 0
...
import io
import re
# Thêm import hàm tạo kết nối Client cho Cloud Supabase
from supabase import create_client 

# =============================================================================
# TẦNG 3 - ĐOẠN 7b (PHẦN 2): RENDERING GIAO DIỆN VÀ ĐỒNG BỘ CLOUD SUPABASE - ĐÃ ĐỒNG BỘ GÕ TAY V33
# =============================================================================
...
# Hiển thị bảng báo cáo kết xuất liên kết 2 chiều
    ...
    if st.button("💾 ĐẨY PHIẾU LÊN HỆ THỐNG TRUNG TÂM CLOUD SUPABASE", use_container_width=True, key="push_to_supabase_cloud_final_btn"):
        try:
            ...
            if not url_direct_push or not key_direct_push: 
                ...
                # 🎯 ĐÃ SỬA: Đọc chính xác tổng sản lượng PO đơn hàng từ bộ nhớ gõ tay toàn cục tránh lỗi NameError
                try:
                    cloud_total_qty = int(float(str(st.session_state.get("current_po_target_val", 0)).replace(",", "")))
                except:
                    cloud_total_qty = 0
