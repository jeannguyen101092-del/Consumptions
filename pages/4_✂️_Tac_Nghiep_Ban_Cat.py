    ...
    if len(lines) >= 2:
        ...
        if len(raw_sizes) == len(raw_qtys) and len(raw_sizes) > 0:
            ...
            if built_dict != st.session_state["manual_parsed_breakdown"]:
                ...
                st.rerun()
# =============================================================================
# TẦNG 3 - ĐOẠN 6 THỐNG NHẤT: Ô LƯỚI TÁC NGHIỆP TỰ DO VÀ LIÊN KẾT THÔNG SỐ V31
# =============================================================================
...
# 🎯 ĐÃ LIÊN KẾT: Ép dòng SẢN LƯỢNG lấy chính xác số lượng PO tổng từ ô nhập thông số phía trên
...
if is_snapshot_valid:
    ...
    for row in filtered_snapshot:
        ...
        # 🎯 CHỪA TRỐNG TUYỆT ĐỐI: Dài sơ đồ lấy nguyên gốc người dùng dán vào, không tự tính
    ...
    for sz in [f"CỠ {i+1}" for i in range(len(active_sizes))]: item_pilot[sz] = "0"
    ...
    for i in range(5):
        ...
        for sz in [f"CỠ {k+1}" for i in range(len(active_sizes))]: item_dict[sz] = "0"
...
def callback_sync_on_the_fly_final():
    if "table_manual_data_editor_final_clean_v1" in st.session_state:
        ...
        if "edited_rows" in st_editor and st_editor["edited_rows"]:
            ...
            # Cộng dồn thời gian thực dòng sản lượng gõ tay nếu người dùng tự sửa đổi
...
# Tính cột Tổng SL động hiển thị trực quan
...
import math
# =============================================================================
# TẦNG 3 - ĐOẠN 7a: ENGINE PYTHON ĐIỀU ĐỘ NGƯỢC THEO CHIỀU DÀI BÀN VẢI MULTI-INSEAM V31
# =============================================================================

# 🎯 CHỈ CHẠY THUẬT TOÁN KHI ĐÃ NHẤN NÚT KÍCH HOẠT ĐỎ TRÊN MÀN HÌNH
if st.session_state.get("c2_normal_cut_btn", False):
    ...
    # 1. Thu thập chính xác số lượng đơn hàng đầu vào từ hàng SẢN LƯỢNG
    if snapshot_current:
    ...
    # Đưa các hàng mốc GIÀNG, SIZE, SẢN LƯỢNG vào danh sách kết quả đầu bảng báo cáo
    for row in snapshot_current:
        if str(row.get("BÀN CẮT / TÊN SƠ ĐỒ", "")).upper().strip() in ["GIÀNG", "SIZE", "SẢN LƯỢNG"]:
            final_snapshot_rows.append(row)

    # 2. Khấu trừ lượng hàng xưởng đã chủ động điền tay (PILOT hoặc sơ đồ mẫu)
    for row in snapshot_current:
        row_name = str(row.get("BÀN CẮT / TÊN SƠ ĐỒ", "")).upper().strip()
        if row_name not in ["GIÀNG", "SIZE", "SẢN LƯỢNG"] and ("PILOT" in row_name or "SS" in row_name):
    ...
    # Vòng lặp rải cuốn chiếu giảm dần lượng hàng
    while sum(current_order_balances.values()) > 0 and marker_counter <= max_safety_loops:
        ...
        for sz in active_sizes:
            ...
            if sz_remaining_qty <= 0:
                base_values[sz] = 0
                continue
            # Chia tỷ lệ phân bổ cuốn chiếu bám sát cơ cấu phần trăm hàng còn lại
        ...
        # Khấu trừ số lượng sản xuất vào mảng dư đơn hàng
        for sz in active_sizes:
            current_order_balances[sz] = max(0, current_order_balances[sz] - (r_dict.get(sz, 0) * row_layers))
            
        # Tạo bản ghi tự động cho dòng sơ đồ chính
        item_auto = {
            "BÀN CẮT / TÊN SƠ ĐỒ": s_marker_name, "SƠ LỚP": row_layers, "SỐ BÀN": 1,
            "DÀI SƠ ĐỒ": 0.0, # 🎯 CHỬA TRỐNG ĐỂ NGƯỜI DÙNG TỰ COPPY TỪ CAD VÀO, KHÔNG TỰ BIA
        ...
        # Giảm dần dung lượng tỷ lệ phối sơ đồ cho vòng sau để vét cuốn chiếu mượt mà
        garments_per_marker = max(2, garments_per_marker - 1)
        marker_counter += 1

    # 4. Giai đoạn cuối cùng: Chèn dòng vét sạch toàn bộ số lượng hàng lẻ còn sót lại
    ...
    # Đẩy toàn bộ mảng kết quả sạch lên kho trung tâm để bảng báo cáo kết xuất
    ...
    # Reset flag giải phóng nút bấm chống vòng lặp vô hạn
