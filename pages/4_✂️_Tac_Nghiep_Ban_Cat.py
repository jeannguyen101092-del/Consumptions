...
import math
# =============================================================================
# TẦNG 3 - ĐOẠN 7a: ENGINE TOÁN HỌC ĐIỀU ĐỘ NGƯỢC THEO CHIỀU DÀI BÀN - FIX NAMEERROR V33
# =============================================================================

# 🎯 THUẬT TOÁN CHỈ CHẠY KHI NGƯỜI DÙNG CHỦ ĐỘNG ẤM NÚT ĐỎ TRÊN MÀN HÌNH
if st.session_state.get("c2_normal_cut_btn", False):
    ...
    # 1. Thu thập chính xác số lượng đơn hàng đầu vào từ hàng SẢN LƯỢNG đã nhập tay/paste Excel
    if snapshot_current:
    ...
    for row in snapshot_current:
        ...
        if row_name not in ["GIÀNG", "SIZE", "SẢN LƯỢNG"] and ("PILOT" in row_name or "SS" in row_name or "C" in row_name):
            ...
            if layers > 0:
                ...
                # Giữ nguyên độ dài sơ đồ người dùng tự coppy dán vào, không tự tính
                final_snapshot_rows.append(row)
    ...
    max_target_layers = 60  # Số lớp rải vải tối đa thông số xưởng quy định

    # 🎯 THUẬT TOÁN ĐIỀU ĐỘ NGƯỢC: Quy định tổng số sản phẩm cực đại trên 1 sơ đồ đi từ Chiều dài tối đa bàn cắt
    # Công thức: Tỷ lệ phối cực đại của sơ đồ đầu tiên = Chiều dài tối đa bàn cắt / Định mức hệ mét
    ...
    while sum(current_order_balances.values()) > 0 and marker_counter <= max_safety_loops:
        ...
        # 🎯 CHỪA TRỐNG TUYỆT ĐỐI Ô DÀI SƠ ĐỒ ĐỂ BẠN TỰ COPPY TỪ CAD VÀO: ĐỂ MẶC ĐỊNH BẰNG 0.0
        item_auto = {
    ...
    # 4. Giai đoạn kết thúc đơn: Chèn dòng vét sạch toàn bộ số lượng hàng đơn lẻ còn sót lại ở đầu khúc
