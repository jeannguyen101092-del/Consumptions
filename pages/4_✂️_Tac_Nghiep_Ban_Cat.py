...
# KIỂM TRA ĐIỀU KIỆN 2: Nếu ĐÃ số hóa xong file SBD -> Màn hình tác nghiệp sản xuất
    ...
    if isinstance(sbd_data_store, dict) and sbd_data_store:
        ...
        if trigger_consumption:
           ...
           # =============================================================================
        # TẦNG 3: ĐỒNG BỘ LUỒNG DỮ LIỆU KHÔI PHỤC SUPABASE VÀ PHÂN TÁCH MA TRẬN PHẲNG
        # =============================================================================
        ...
        if cutting_source_data is not None:
            ...
            if isinstance(cutting_source_data, list):
                valid_items = [i for i in cutting_source_data if str(i.get("Sơ đồ / Trạng thái", "")).strip().lower() != "balance"]
            else:
                valid_items = []
            for item in valid_items:
                ...
                for idx, sz in enumerate(active_sizes):
                    current_ratio = int(ratios_dict.get(sz, 0))
                ...
                with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
                    ...
                    for r_idx in range(len(df_final_report)):
                        ...
                        for c_idx, val in enumerate(row_vals):
                            if r_idx <= 2: worksheet.write(r_idx + 1, c_idx, val, f_admin)
                            elif r_idx == 3: worksheet.write(r_idx + 1, c_idx, val, f_giang)
                            elif r_idx == 4: worksheet.write(r_idx + 1, c_idx, val, f_size)
                            elif r_idx == 5: worksheet.write(r_idx + 1, c_idx, val, f_sl_po)
                            elif "CÒN LẠI" in row_title: worksheet.write(r_idx + 1, c_idx, val, f_con_lai)
                            else:
                                if c_idx == 0: worksheet.write(r_idx + 1, c_idx, val, f_title_col)
                                elif c_idx <= len(active_sizes) and pd.notna(val) and str(val).replace('.','',1).isdigit() and int(float(val)) > 0:
                                    worksheet.write(r_idx + 1, c_idx, int(float(val)), f_ratio_active)
                                else: worksheet.write(r_idx + 1, c_idx, int(float(val)) if str(val).replace('.','',1).isdigit() else val, f_tyle_cell)
                                    
                st.download_button(label="📥 IN FILE EXCEL ĐỔ MÀU PHÂN KHỐI CÔNG NGHIỆP (MẪU TNC-DNA CHUẨN)", data=buffer.getvalue(), file_name=f"PHIEU_TAC_NGHIEP_BÀN_CẮT_MÀU_{style_id_input}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, key="excel_download_btn_final_v120_colorful")
            except Exception: pass
            st.markdown("""<style>
            ...
            if trigger_save_supabase:
                with st.spinner("🚀 Hệ thống đang đồng bộ lên Cloud Supabase..."):
                    ...
                    try:
                        ...
                        st.success(f"🎉 Đã lưu trữ thành công phiếu tác nghiệp mã hàng {style_id_input} lên Cloud Supabase!")
                    except Exception as e: st.error(f"⚠️ Lỗi kết nối Supabase: {str(e)}")
            st.markdown("---")
            st.success("🎉 Toàn bộ phân hệ gọi dữ liệu, ghim trần và xuất file màu công nghiệp đã đồng bộ thành công!")
        else:
            st.info("💡 Quy trình: Chọn phiếu tác nghiệp cũ từ Supabase HOẶC tải file SBD mới ở màn hình ngoài.")
