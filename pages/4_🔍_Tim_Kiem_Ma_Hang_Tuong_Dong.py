...
# =====================================================================
# 17. GEMINI SEMANTIC EMBEDDING (ĐÃ SỬA LỖI 404 THEO SDK MỚI NHẤT)
# =====================================================================

def get_image_embedding(text_content: str):
    """
    Tạo vector nhúng ngữ nghĩa (768 chiều) từ văn bản mô tả kỹ thuật.
    Sử dụng hàm client.models.embed_content chuẩn cấu trúc SDK google-genai.
    """
    try:
        # Sử dụng đúng cú pháp của SDK mới: Truyền text trực tiếp vào tham số contents
    ...
    try:
        # CẤU TRÚC ĐÚNG: Trong SDK mới, kết quả trả về là một object,
        # bên trong có thuộc tính 'embeddings' là một LIST các đối tượng ContentEmbedding.
        # Mỗi đối tượng ContentEmbedding lại chứa thuộc tính 'values'.
        if response.embeddings and len(response.embeddings) > 0:
            values = response.embeddings[0].values
        else:
            raise Exception("Không tìm thấy dữ liệu vector trong phản hồi từ Google.")
    ...
    # ---------------------------------------------------------
    # NORMALIZE VECTOR (Chuẩn hóa để lưu vào pgvector)
    # ---------------------------------------------------------
    norm = math.sqrt(
        sum(
            float(x) * float(x)
...
#   3. AI Vision phân tích cấu trúc garment
#   4. Text Embedding từ AI analysis
#   5. Lưu embedding vào Supabase
#   6. Tìm mã tương đồng
#   7. Category chỉ BOOST, không khóa search
#   8. Chống lỗi 429 / 503
#   9. Fallback an toàn khi Gemini tạm lỗi
# =====================================================================
...
with tab_storage:
    ...
    st.info(
        ...
        "Embedding được tạo từ phân tích ngữ nghĩa của AI."
                        ...
                        ai_result = (
                            create_fallback_ai_result(
                                ...
                                reason=(
                                    ...
                                    "Gemini Vision "
                                    "không phản hồi. "
                                    "Sử dụng category kho "
                                    "làm dữ liệu dự phòng."
...
#
# FIX:
# - AI Vision nhận dạng garment
# - Image Embedding nhận IMAGE BYTES
# - Không truyền text vào image embedding
# - Có retry chống 429 / quá tải Gemini
# - Delay giữa các file
# - Không xóa dữ liệu đã lưu trong DB / Storage
# - Chỉ xóa hàng đợi sau khi xử lý
# =====================================================================
        ...
        for i, file in enumerate(
            ...
            with preview_cols[
                ...
                try:
                    ...
                    st.warning(
                        f"Không preview được: {file.name}"
                    )
        ...
        if st.button(
            ...
            for index, file in enumerate(
                ...
                try:
                    ...
                    # QUAN TRỌNG:
                    #
                    # Hàm này nhận IMAGE BYTES.
                    #
                    # KHÔNG:
                    #
                    # get_image_embedding(reason)
                    #
                    # =================================================
