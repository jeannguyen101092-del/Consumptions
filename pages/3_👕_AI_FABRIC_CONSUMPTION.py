...
if rows is not None and (isinstance(rows, list) and len(rows) > 0 or isinstance(rows, pd.DataFrame) and not rows.empty):
    ...
    weft_shrink = float(st.session_state["weft_shrink"])
       ...
       # =====================================================================
       # =====================================================================
    # 🟩 ĐOẠN 3.1: AI MULTI-LAYER PRODUCT CLASSIFIER (BỘ PHÂN TÍCH LOẠI HÀNG CHUẨN)
    # =====================================================================
    import pandas as pd
    ...
    # Nếu bộ rập chứa các chi tiết đặc trưng của Quần như Cạp, Dải cạp, Ống quần, Đáy, Đũng...
    ...
    # Nếu không có từ khóa quần và chứa các từ khóa đặc trưng của Áo có tay/cổ mổ khóa...
    ...
    if 'COMPANY_DENSITY_PRIOR' in locals() and product_category in COMPANY_DENSITY_PRIOR:
        ...
        base_prior = 0.875 if product_category == "JEAN_LONG" else 0.78
    ...
    def _d3_internal_material_classify(row, idx, prod_cat):
        ...
        # 🚨 ĐÃ SỬA: Loại bỏ WAISTBAND, LƯNG, CẠP ra khỏi bộ lọc mếch keo danh định
        fusing_kws = ["FUSING", "INTERLINING", "KEO", "MEC", "MẾCH", "RIB", "BOND", "RIB", "ADHESIVE", "LOT KEO", "TRICOT"]
        lining_kws = ["LINING", "LOT", "LÓT", "POCKETING", "MESH", "TAFFETA", "VAI LOT", "VẢI LÓT"]
        
        # Nếu linh kiện là cạp quần (WAISTBAND) nhưng chất liệu vải chính thì bắt buộc phải giữ lại nhóm FABRIC
    ...
    for idx, r in df_bom.iterrows():
        ...
        # Lọc bỏ hoàn toàn các dòng phụ liệu kim loại/nhãn mác khỏi luồng đo hình học
    ...
    # Bảo vệ trần/sàn mật độ hồi quy cho Quần dài
    if product_category == "JEAN_LONG":
        estimated_density = max(0.8150, min(0.94, base_prior + density_delta))
    else:
    ...
    ai_complexity = "COMPLEX" if complexity_score >= 65 else "NORMAL"

    calculated_wastage = 1.015 + ((features["stripe_plaid_flag"] * 0.02) + (features["one_way_flag"] * 0.01) + (0.01 if fabric_type in ["KNIT", "THUN"] else 0.0))
    ...
    with m3: st.metric(label="🧩 Tổng số mảnh rập thực tế", value=f"{features['total_pieces']:.0f} Pcs")
       ...
       # =====================================================================
    # 🟩 ĐOẠN 4: AI VIRTUAL PIECE ENGINE & GEOMETRIC PREPROCESSOR (NO ACCESSORY)
    # =====================================================================
    ...
    for idx, row in df_bom.iterrows():
        ...
        # 🚨 CỨU HỘ DIỆN TÍCH TUYỆT ĐỐI KHÔNG SẬP SỐ 0
        net_area_raw = float(row.get("polygon_net_area", 0.0))
        if net_area_raw <= 0.0:
            net_area_raw = l_orig * w_orig * 0.76 if (l_orig > 0 and w_orig > 0) else 1.0
            
        bbox_area_raw = l_orig * w_orig if (l_orig > 0 and w_orig > 0) else 1.0
        ...
        if "user_edited_materials" in st.session_state and idx in st.session_state["user_edited_materials"]:
            ...
            # 🚨 NẾU GẶP TỪ KHÓA PHỤ LIỆU, CHỈ, NÚT, ZIPPER -> ĐÁNH DẤU LÀ ACCESSORY ĐỂ KHỬ TRỪ
            ...
            elif slenderness >= 5.0 and void_ratio <= 0.15 and l_orig > 0:
    ...
    ctx["ai_expert_decision"]["virtual_pieces_layer"] = virtual_pieces_layer
       # =====================================================================
    # 🟩 ĐOẠN 5.1: GEOMETRIC MARKER ENGINE (ÉP ĐỊNH MỨC QUẦN XUỐNG THẤP)
    # =====================================================================
    ...
    # 🚨 ĐIỀU CHỈNH 1: Hạ hệ số hao hụt đầu bàn động từ 1.03 (3%) xuống mốc chặt chẽ 1.015 (1.5%) cho dòng Quần
    ...
    if is_trouser:
        target_wastage = 1.015  # Ép sàn hao hụt chặt chẽ cho Quần dài
        ctx["ai_expert_decision"]["product_category"] = "JEAN_LONG"
    ...
    for idx, r in df_bom.iterrows():
        ...
        elif any(k in comp_name_str for k in ["BUTTON", "ZIP", "THREAD", "NÚT", "CHỈ"]):
        ...
        if v_piece.get("inferred_class", "FABRIC") == "FABRIC":
            ...
            if v_piece.get("production_l", 0) > 0:
                for _ in range(int(current_pcs)):
                    fabric_pieces_to_nest.append({
                        "l": v_piece["production_l"], 
                        "w": v_piece["production_w"], 
                        "area": net_area
                    })
    ...
    if len(fabric_pieces_to_nest) > 0 and current_fabric_width > 0:
        ...
        min_density_offset = -0.02
        max_density_offset = 0.05  
        mean_piece_area = total_fabric_net_area / len(fabric_pieces_to_nest)
        
        size_penalty = 0.0
        if mean_piece_area > 180.0:  
            size_penalty = min(0.02, (mean_piece_area - 180.0) * 0.0001) # Nới lỏng độ phạt kích thước
            real_fabric_density -= size_penalty  
            min_density_offset -= size_penalty   
            
        if is_trouser:
            real_fabric_density += 0.03          # Đẩy nhẹ hiệu suất nền
            max_density_offset += 0.03           
        ...
        real_fabric_density = max(estimated_density_prior + min_density_offset, min(estimated_density_prior + max_density_offset, real_fabric_density))
        
        # 🚨 ĐIỀU CHỈNH 2: Nâng nhẹ khung trần hiệu suất từ [80.5% - 84.5%] lên vùng tối ưu khít khao [84.0% - 85.5%]
        if is_trouser:
            real_fabric_density = max(0.8400, min(0.8550, real_fabric_density))
        else:
            real_fabric_density = max(0.5800, min(0.9200, real_fabric_density))
    ...
    # =====================================================================
    # 🟩 ĐOẠN 5.2: CONSUMPTION ROUTER & PUBLISHING (ÉP SÁT ĐỊNH MỨC DÒNG)
    # =====================================================================
    ...
    if len(fabric_pieces_to_nest) > 0:
        st.success(f"🧩 **GEOMETRIC SOLVER KẾT QUẢ** | Mật độ tối ưu (Real Density): `{real_fabric_density*100:.2f}%` | Định mức vải chính: `{total_fabric_gross_yds:.3f} Yds` (Đã ép chặt định mức dòng chi tiết)")
