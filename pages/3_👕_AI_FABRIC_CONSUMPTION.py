...
if rows is not None and (isinstance(rows, list) and len(rows) > 0 or isinstance(rows, pd.DataFrame) and not rows.empty):
    ...
    for idx, vp in virtual_pieces_layer.items():
       ...
       # =====================================================================
    # 🟩 ĐOẠN 5.1 (PHIÊN BẢN V25 - ĐỒNG BỘ SỐ LƯỢNG RẬP XOÁ LỖI PCS=1): MAXRECTS PREPARATION
    # =====================================================================
    import json
    import math  
    ...
    # 🌟 ĐỒNG BỘ TÊN CỘT TỪ KEYWORDS GIAO DIỆN THỰC TẾ
    ...
    for idx, r in df_bom.iterrows():
        ...
        # Đọc dữ liệu kích thước rập an toàn
        ...
        # Tự động bù bình diện tích tinh từ hộp bao nếu PDF khuyết
        if net_area <= 0 and p_len > 0 and p_wid > 0:
            net_area = p_len * p_wid * (0.76 if "FABRIC" in p_class_check else 0.85)
        
        # 🌟 SỬA ĐOẠN ĐỌC SỐ LƯỢNG: Bắt chuẩn cột "Số lượng rập" từ giao diện UI
        raw_pcs = float(r.get(pcs_col, 1.0)) if pcs_col else 1.0
        inferred_pcs = raw_pcs
        
        # Tháp lọc AI tự động nhân đôi chi tiết vế (Trái/Phải) nếu PDF Techpack bị thiếu thông tin vế
        if inferred_pcs == 1.0 and p_class_check in ["FABRIC", "LINING"]:
            if any(k in c_name_upper for k in ["LEG", "THAN", "ỐNG", "PANEL", "POCKET BAG", "TÚI TRƯỚC"]):
                if not any(k in c_name_upper for k in ["LEFT", "RIGHT", "TRÁI", "PHẢI", " (L)", " (R)"]):
                    inferred_pcs = 2.0

        # Lấy giá trị sau cùng (Ưu tiên nếu người dùng chủ động sửa tay trực tiếp trên lưới UI)
        pcs = float(st.session_state.get("user_edited_pieces", {}).get(idx, inferred_pcs))
        
        # Đẩy giá trị số lượng rập chuẩn (2 cái) đồng bộ ngược lại df_bom nền để render lên màn hình
        if pcs_col:
        ...
        # Nhân quy đổi số lượng sơ đồ theo hệ số nhảy size (Size Scale Ratio)
        pcs = pcs * size_scale_ratio

        # Đồng bộ xử lý hình học rập bản rộng
        ...
        if p_class_check in ["FABRIC", "FUSING", "INTERLINING", "LINING", "RIB"] and p_len > 0:
            ...
            # TÍCH HỢP AI PAIR BUILDER CORE: Đóng gói cặp đối xứng Trái/Phải triệt tiêu không gian trống chết
    ...
    # Sắp xếp sơ đồ đa tiêu chí (Ưu tiên cụm siêu rập Pair xếp trước, rập lẻ xếp sau theo diện tích giảm dần)
    ...
    total_fabric_gross_yds = (simulated_marker_length / 36.0) * 1.030
       ...
       # =====================================================================
    # 🟩 ĐOẠN 5.2 - PHẦN B: PUBLISHING ROUTER (PHIÊN BẢN OVERRIDE TÁCH VẬT TƯ)
    # =====================================================================
    ...
    # 🔥 BƯỚC OVERRIDE: Ép tách phân loại vật tư chuẩn xác bằng tên linh kiện thực tế từ PDF
    for idx, row in df_bom.iterrows():
        ...
        # Nếu tên linh kiện chứa từ khóa của keo/mếch dựng hình
        if any(x in c_nm_upper for x in ["FUSING", "MEC", "MẾCH", "KEO", "INTERLINING", "INTERFACING"]):
            p_cls = "FUSING"
        # Nếu tên linh kiện chứa từ khóa của lót túi/vải lót/bo gân
        elif any(x in c_nm_upper for x in ["LINING", "LÓT", "POCKET BAG", "POCKETING", "RIB"]):
            p_cls = "LINING"
            
        # Ghi nhận ngược lại DataFrame lưới UI để hiển thị trực quan cho người dùng xem
        if m_class_col:
            df_bom.at[idx, m_class_col] = p_cls

    # 2. VÒNG LẶP GOM DIỆN TÍCH ĐỘC LẬP THEO TỪNG LỚP VẬT TƯ SAU KHI ĐÃ OVERRIDE TÁCH BIỆT
    for idx, row in df_bom.iterrows():
        ...
        # Gom diện tích riêng biệt vào từng khoang bộ nhớ vật tư, giải phóng cho vải chính
    ...
    # 3. Quy đổi định mức tổng Gross Yds độc lập cho các nhóm vật tư phụ
    ...
    # Hiệu chỉnh lại simulated_marker_length vải chính (Trích xuất các linh kiện phụ đã tách ra khỏi sơ đồ vải chính)
    if tot_fb_area > 0 and real_fabric_density > 0 and current_fabric_width > 0:
        # Chiều dài sơ đồ thực tế chỉ chứa các chi tiết FABRIC thật sự
        optimized_marker_len = (tot_fb_area / current_fabric_width) / real_fabric_density
        total_fabric_gross_yds = (optimized_marker_len / 36.0) * 1.030
    ...
    # 4. Phân bổ chỉ số Gross Consumption cho từng dòng rập dựa trên mảng diện tích sạch
    gross_list = []
    for idx, row in df_bom.iterrows():
        v_pc = virtual_pieces_layer.get(idx, {}) if isinstance(virtual_pieces_layer, dict) else {}
        p_cls = str(row.get(m_class_col, "FABRIC")).upper().strip()
    ...
    # 5. Cập nhật mảng giá trị định mức thương mại sạch lên giao diện
