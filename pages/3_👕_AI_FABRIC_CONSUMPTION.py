...
if rows is not None and (isinstance(rows, list) and len(rows) > 0 or isinstance(rows, pd.DataFrame) and not rows.empty):
    ...
    st.session_state["bom_data"] = ctx
    # =====================================================================
    # 🟩 ĐOẠN 5.1 (PHIÊN BẢN V32 - SỬA LỖI LỀ ĐỒNG BỘ): MULTI-MARKER PREPARATION
    # =====================================================================
    import json
    ...
    for idx, r in df_bom.iterrows():
        ...
        # SỬA LỖI DIỆN TÍCH TINH: Bóc đúng nhãn diện tích tinh của phôi ảo từ RAM Đoạn 4 đổ sang
    ...
    # =====================================================================
    # 🟩 ĐOẠN 5.2 (PHIÊN BẢN V32 - FIX SẬP LỀ ĐỒNG BỘ): THREE INDEPENDENT ENGINES
    # =====================================================================
    ...
    # ➔ ĐỘNG CƠ SƠ ĐỒ 1: Đi sơ đồ độc lập cho VẢI CHÍNH (MaxRects BSSF Core)
    ...
    # ➔ ĐỘNG CƠ SƠ ĐỒ 2: Đi sơ đồ độc lập cho MÉC / KEO (MaxRects BSSF Core)
    ...
    # ➔ ĐỘNG CƠ SƠ ĐỒ 3: Đi sơ đồ độc lập cho VẢI LÓT TÚI (MaxRects BSSF Core)
    ...
    st.success(f"💎 **CAD V32 TRIPLE ENGINES COMPLETE** | Đồng bộ diện tích tinh thành công | Định mức vải chính tối ưu biệt lập: `{total_fabric_gross_yds:.3f} Yds`")
