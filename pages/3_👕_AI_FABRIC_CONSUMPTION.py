...
if rows is not None and (isinstance(rows, list) and len(rows) > 0 or isinstance(rows, pd.DataFrame) and not rows.empty):
    ...
    # =====================================================================
    # 🟩 ĐOẠN 5.2: CONSUMPTION ROUTER & PUBLISHING (ĐỒNG BỘ TUYỆT ĐỐI)
    # =====================================================================
    ...
    def core_engine_router(row, idx):
        ...
        # 1. Định tuyến phân bổ cho nhóm Vải chính & Vải phối
        ...
        # 2. Định tuyến phân bổ cho nhóm Vải lót (Đã vá đồng bộ hoàn toàn với bao túi túi trước)
        if p_cls == "LINING":
            ...
            # Luồng dự phòng nếu sơ đồ tổng vải lót gặp sự cố, tự động phân bổ theo diện tích đơn lẻ
            return round((((net_area * pcs) / lining_width / 0.80) / 36.0) * target_wastage, 4) if lining_width > 0 else 0.0
            
        # 3. Định tuyến phân bổ cho nhóm Keo / Mếch dựng / Bo rib
    ...
    # Đẩy dữ liệu định mức sạch sau khi tính toán vào DataFrame của bảng BOM
    gross_consumptions = [core_engine_router(row, idx) for idx, row in df_bom.iterrows()]
    df_bom["Gross Consumption"] = gross_consumptions
    
    # Đồng bộ hiển thị đúng khổ vải sản xuất của dòng chi tiết dựa trên phân loại thực tế
    ...
    # In thông báo tổng hợp kết quả định mức sơ đồ tổng lên UI Streamlit
    if len(fabric_pieces_to_nest) > 0:
        st.success(f"🧩 **GEOMETRIC SOLVER**: Vải chính (`{real_fabric_density*100:.1f}%`): `{total_fabric_gross_yds:.3f} Yds` | Lót: `{total_lining_gross_yds:.3f} Yds` | Keo: `{total_fusing_gross_yds:.3f} Yds`")
