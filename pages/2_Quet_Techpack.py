...
with control_col2:
    ...
    if st.button("🗑️ XÓA LỊCH SỬ CHAT", use_container_width=True, type="secondary"):
        ...
        st.success("🔄 MEMORY PURGED"); time.sleep(0.5); st.rerun()

st.markdown("<hr style='border-color:#E2E8F0; margin: 20px 0;'>", unsafe_allow_html=True)

if "chat_history" not in st.session_state:
    st.session_state["chat_history"] = [{"role": "assistant", "type": "text", "content": "Welcome to PPJ Textile Visual R&D Engine. Hãy tải lên sơ đồ rập/Techpack mã mới, hệ thống sẽ bóc tách biệt lập, tự động đối soát kho Master DB và lập luận toán học để kết xuất định mức vải cùng bảng phụ liệu BOM chính xác."}]
for msg in st.session_state["chat_history"]:
    with st.chat_message(msg["role"]): st.write(msg["content"])
                    ...
                    # Bộ bẫy lỗi lũy tiến Backoff 5 lần tránh sập mạng khi tải tác vụ nặng
                    ans = ""
