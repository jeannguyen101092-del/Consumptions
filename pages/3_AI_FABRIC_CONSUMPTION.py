...
# Hiển thị lịch sử chat chuẩn định dạng giao diện
...
# 🌟 HÀM XỬ LÝ KHI NGƯỜI DÙNG NHẤN ENTER (GIẢI QUYẾT LỖI LẶP CHAT)
def process_chat_input():
    user_prompt = st.session_state.chat_widget_input.strip()
    if not user_prompt:
        return
        
    # Bóc tách thông số toán học từ chuỗi văn bản (NLP Parser)
    ...
    # Logic tính toán mô phỏng dựa trên tham số lệnh
    ...
    # Áp hệ số co rút dệt phẳng thực tế vào định mức
    ...
    # Cập nhật các khối Metric đồng bộ lên màn hình chính
    ...
    # Lưu thông tin vào lịch sử hiển thị
    ...
    # 🌟 QUAN TRỌNG: Làm rỗng widget input ngay sau khi xử lý xong câu lệnh để tránh lặp
    st.session_state.chat_widget_input = ""

# Ô nhập liệu sử dụng on_change callback để dọn dẹp biến tạm thời
st.text_input(
