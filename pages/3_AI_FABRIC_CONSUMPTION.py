...
def parse_geometric_panels_allowance(ai_blueprint: dict, user_chat: str) -> dict:
    ...
    NÂNG CẤP V16.2: Gia cố bẫy lỗi kiểm tra phần tử rỗng (None Type) để chống tuyệt đối lỗi sập hệ thống.
    """
    ...
    # BỘ CẤU HÌNH SHAPE FACTOR ĐỘNG THEO BIÊN DẠNG CHI TIẾT GERBER THỰC TẾ
    SHAPE_FACTORS = {
        "FRONT": 0.54,      # Thân trước quần
        "BACK": 0.59,       # Thân sau quần
        "WAISTBAND": 0.94,  # Cạp lưng quần
        "POCKET": 0.78,     # Lót túi/Đáp túi
        "SLEEVE": 0.64,     # Tay áo
        "DEFAULT": 0.62     # Linh kiện thông thường
    }
    ...
    for row in all_rows:
        ...
        # BỘ LỌC CHẶN TRƯỚC: Loại trừ phụ liệu phần cứng (chỉ, nhãn, nút, khóa...)
        ...
        if panels and isinstance(panels, list):
            for panel in panels:
                # 🟢 CHỐT BẢO VỆ TUYỆT ĐỐI CHỐNG SẬP: Nếu panel bị rỗng (NoneType) thì bỏ qua ngay, không chạy lệnh .get()
                ...
                # Áp hệ số biên dạng Gerber thực tế
                ...
                # A. TÍNH TOÁN CHU VI THỰC (TRUE PERIMETER) VÀ DIỆN TÍCH TỪ TỌA ĐỘ POLYGON CAD
                ...
                if polygon_points and isinstance(polygon_points, list) and len(polygon_points) >= 3:
                    ...
                    # THUẬT TOÁN TRUE PERIMETER: Cộng dồn khoảng cách Euclid giữa các điểm nối tiếp
                ...
                # Bù đường may dựa trên chu vi thực
...
def parse_geometric_panels_allowance(ai_blueprint: dict, user_chat: str) -> dict:
    ...
    SỬA ĐỔI TOÁN HỌC: Hạ hệ số bao rập quần về 0.52 để triệt tiêu diện tích trống vùng đáy và ống.
    """
    product_type = str(ai_blueprint.get("detected_product_type", "DEFAULT")).upper().strip()
        ...
        if panels:
            for panel in panels:
                ...
                if polygon_points and isinstance(polygon_points, list) and len(polygon_points) >= 3:
                    ...
                    # 📌 ĐIỂU CHỈNH LÕI DIỆN TÍCH: Hạ từ 0.88 xuống 0.52 cho đúng biên dạng vát cong của quần dài Tây/Jeans thực tế
                    base_area = p_len * p_wid * 0.52 if (p_len > 0 and p_wid > 0) else 0.0
                
                if base_area == 0.0:
                    # Phương án dự phòng (Fallback) nếu thiếu kích thước chi tiết từ AI
                    eval_len_fb = outseam_length if product_type in ["PANT", "CARGO_PANT", "CAPRI_PANT", "JORT"] else body_length
                    eval_wid_fb = hip_width if product_type in ["PANT", "CARGO_PANT", "CAPRI_PANT", "JORT"] else chest_width
                    base_area = (eval_len_fb * eval_wid_fb * 0.20) # Hạ tỷ trọng vùng diện tích mặc định
...
def allocate_fabric_consumption_and_quality_gate(ai_blueprint: dict) -> dict:
    ...
    for f_id, data in fabric_registry.items():
        ...
        for row in data["rows_to_update"]:
            ...
            # Chặn rò rỉ nếu dòng vật tư cứng nằm lọt vào nhóm registry
