...
# =====================================================================
# ĐOẠN 2a1: RẬP HÌNH HỌC, TỰ ĐỘNG BÙ TRỪ & LOG CHUYÊN SÂU (V16.3 FIX CRASH)
# =====================================================================
...
def parse_geometric_panels_allowance(ai_blueprint: dict, user_chat: str) -> dict:
    ...
    SỬA ĐỔI: Chống lỗi 'NoneType' crash bằng cơ chế xác thực kép loại trừ phần tử null.
    """
    ...
    for row in all_rows:
        ...
        if panels and isinstance(panels, list):
            for panel in panels:
                # 🟢 BẢO VỆ CHỐT CHẶN (FIX CRASH): Xác thực phần tử bắt buộc phải là Dictionary, loại bỏ hoàn toàn NoneType/String gây sập nguồn
...
def execute_marker_yardage_and_quality_gate(ai_blueprint: dict, user_chat: str) -> dict:
    ...
    for row in all_rows:
        ...
        if tmp_id not in fabric_registry:
            ...
            # CHỐT SỬA ĐỐI XỨNG PHÒNG CẮT: Nếu mã Fabric ID này đã nhận diện diện tích toàn bộ 4 thân từ trước,
            # kiểm tra giá trị bằng nhau của các dòng để loại bỏ không cộng lặp chồng diện tích một lần nữa.
            if abs(fabric_registry[tmp_id]["accumulated_area_sq_in"] - row["_computed_net_area_sq_in"]) > 15.0:
                # Chỉ cộng dồn thêm khi phát hiện đây là cấu kiện hoàn toàn khác biệt (ví dụ vải phối túi rời)
                fabric_registry[tmp_id]["accumulated_area_sq_in"] += row["_computed_net_area_sq_in"]
