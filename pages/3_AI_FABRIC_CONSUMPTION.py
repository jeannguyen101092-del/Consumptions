...
# =====================================================================
            ...
            """
# =====================================================================
# ĐOẠN 7a - PHẦN 3: DUAL-AGENT AI AUDITING MIDDLEWARE (V90.0 PRODUCTION)
# 🌟 TẦNG 1: TRÍCH XUẤT HÌNH HỌC CAD -> TẦNG 2: KIỂM TOÁN LOGIC CHỐT SỐ YARDS
# 🌟 ĐẬP BỎ TOÀN BỘ ENGINE TÍNH TOÁN CŨ - PYTHON CHỈ TIẾP NHẬN HIỂN THỊ
# =====================================================================
            response_text = ""
            ...
            # CHỈ KÍCH HOẠT CHUỖI PHẢN ỨNG AI KHI CÓ THAY ĐỔI COMMAND HOẶC CHƯA CÓ DỮ LIỆU
            if has_no_data_p3 or is_signature_changed_p3:
                try:
                    # -----------------------------------------------------------------
                    # 🧠 TẦNG 1: AGENT TRÍCH XUẤT DIỆN TÍCH PANEL SƠ BỘ
                    # -----------------------------------------------------------------
                    prompt_agent_1 = f"""
                    You are Agent 1: A Textile CAD Geometry Specialist. 
                    Your job is to scan the techpack, identify the product type, extract all relevant measurements for size '{target_size_cmd}', and calculate the raw surface area of each fabric panel including shrinkage.
                    ...
                    Your job is to review the temporary technical JSON generated by Agent 1, cross-check it with the physical reality of garment production, and output the absolute FINAL verified consumption matrix.

                    🌟 AUDITOR CHECKLIST & RULES:
                    1. Product Logic: If product is '{target_size_cmd}', the Main Fabric gross consumption MUST realistically range between 1.15 and 1.85 yards. If it's too low (e.g. < 1.0 yds), you MUST flag and scale it up based on standard marker layouts.
                    2. Component Logic: Fusing or Lining areas cannot exceed the Main Fabric area. Ensure ALL material types from BOM are covered.
                    3. Verification: Apply a safety factor for wastage and end-of-roll allowance. Ensure 'gross_consumption' is a precise, mathematically verified float.

                    Output BOTH raw plain text JSON format and a friendly markdown chat response using the exact markers below without markdown code blocks. All 'fabric_width_inch' MUST match the value {active_width}:
                    
                    ===START_CHAT===
                    ⚖️ **IE Auditor Verified**: Định mức đã được thẩm định qua hệ thống AI 2 tầng bảo vệ. Kết quả cho **Size {target_size_cmd}** (Khổ {active_width}\", Co rút: Dọc {warp_val}/Ngang {weft_val}) đã chốt cấu trúc an toàn thành công, giảm thiểu tối đa sai số biến động.
                    ===END_CHAT===
                    ...
                    {{
                      ...
                      "bom_rows": [
                        {{
                          ...
                          "marker_efficiency": "88.9%",
                          "gross_consumption": 1.425,
                          "reasoning": "Auditor Confirmed: Scaled up from raw panel areas to fit {active_width} inch width marker layout with safety wastage factor applied."
                        }},
                        {{
                          "component_type": "Fusing",
                          "fabric_classification": "FUSING",
                          ...
                          "marker_efficiency": "88.9%",
                          "gross_consumption": 0.215,
                          "reasoning": "Auditor Confirmed: Verified block reinforcement templates layout matching pocket and waist components."
                        }}
                      ]
                    }}
                    ===END_JSON===
            ...
            if active_json_stream:
                ...
                if raw_json_str:
                    ...
                    if blueprint_worker and "bom_rows" in blueprint_worker:
                        ...
                        # Đồng bộ thông số co rút động đã trích xuất vào từng dòng dữ liệu kỹ thuật
                        for row in blueprint_worker.get("bom_rows", []):
                            row["fabric_width_inch"] = active_width
                            row["_btp_warp_pct"] = warp_val
                            row["_btp_weft_pct"] = weft_val
                        
                        # Đẩy thẳng qua cổng tiếp nhận an toàn để cập nhật trạng thái UI
                        ...
                        if response_text:
                            ...
                            st.rerun() # Ép tải lại trang hiển thị dữ liệu mới lập tức
                    else:
