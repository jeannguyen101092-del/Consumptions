    ...
    with st.chat_message("assistant"):
        with st.spinner("Hệ thống AI R&D Engine đang kết nối kho tri thức nền dệt may..."):
            ...
            if not gemini_key: 
                ...
                analysis_prompt = f"You are an expert Apparel Costing Engineer. Perform a strict fabric consumption analysis. [PRODUCTION VARIABLES] - Target Usable Fabric Width: {user_width} - Target Fabric Shrinkage Weft (Ngang): {co_ngang}% - Target Fabric Shrinkage Warp (Dọc): {co_doc}% [SPECS DATA] - New Style Uploaded Measurements: {json.dumps(new_style_measurements_dict, ensure_ascii=False)} - Warehouse Match Techpack Specs: {json.dumps(db_measurements_raw, ensure_ascii=False)} - Warehouse Match Fabric Records: {json.dumps(fabric_records, ensure_ascii=False)} [CRITICAL EXECUTION MODE: {pipeline_mode}] RULE 1: IF MODE IS 'HISTORY_BASED_PREDICTION': Locate 'Định mức gốc' of matched style in warehouse. Compare New Style against Warehouse Specs to calculate delta. Adjust 'Định mức gốc' proportionally. State: 'Dự đoán định mức dựa trên mã tương đồng lịch sử'. RULE 2: IF MODE IS 'PURE_MARKER_CALCULATOR': Calculate from scratch using standard apparel marker geometry. Add 5% wastage. State: 'Tính toán độc lập theo quy ước hình học ngành may do không có mã tương đồng'. [OUTPUT FORMAT] Output step-by-step completely in Vietnamese. Keep it strictly technical, professional and precise."
