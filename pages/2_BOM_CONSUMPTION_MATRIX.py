...
if 'menu_selection' in globals() and menu_selection == "🧵 BOM & Consumption Matrix":
    ...
    st.markdown('<div class="component-title-box">🧵 INTELLIGENT BOM & MATRIX</div>', unsafe_allow_html=True)
    ...
    if "pdf_raw_text" not in st.session_state: st.session_state["pdf_raw_text"] = ""
    ...
    with control_col1:
        ...
        temp_bytes, new_file_hash = None, None
        if uploaded_file is not None:
            try:
                uploaded_file.seek(0)
                temp_bytes = uploaded_file.read()
                new_file_hash = hashlib.md5(temp_bytes).hexdigest()
                uploaded_file.seek(0)
            except Exception: pass

        if uploaded_file is not None and new_file_hash != st.session_state["current_file_hash"]:
    ...
    if not has_file:
        st.info("👋 Vui lòng tải lên tệp Techpack hồ sơ thiết kế (PDF/Hình ảnh) ở phía trên.")
        st.stop()
    ...
    uploaded_file_name = st.session_state.get("previous_uploaded_file_name", "")

    st.info("🛠️ PRODUCTION DIAGNOSTIC ENGINE:")
    st.write(f"📌 **FILE:** `{uploaded_file_name}` | **HASH:** `{st.session_state['current_file_hash']}`")
    st.write(f"📏 **SESSION LEN:** `{len(st.session_state.get('visual_description_str', ''))}` | **GLOBAL LEN:** `{len(str(globals().get('new_style_sketch_vector', '')))}`")

    vision_page_index = 0

    if st.session_state["matched_techpack"] is None:
        if ("pdf" in str(detected_mime_type).lower() or uploaded_file_name.lower().endswith(".pdf")) and target_new_sketch_bytes and not st.session_state["pdf_raw_text"]:
            try:
                import fitz
                ...
                try:
                    ...
                    if len(extracted_text.strip()) > 50: st.session_state["pdf_raw_text"] = extracted_text
                finally: doc.close()
            except Exception as pdf_extract_err: st.warning(f"Lỗi cào text: {pdf_extract_err}")
        ...
        if len(new_vec) < 30 and target_new_sketch_bytes and client and client.models:
            with st.spinner("🔄 Đang quét ảnh tái lập Sketch Vector..."):
                try:
                    ocr_prompt = "Extract apparel construction. Return format strictly: GARMENT_TYPE: PANT\nFEATURES:\nZIP FLY"
                    
                    # 🚀 BỘ QUÉT TỰ ĐỘNG CHỌN TRANG PDF SẠCH ĐÃ ĐƯỢC NHÚNG CHUẨN XÁC VÀO ĐÂY
                    if "pdf" in str(detected_mime_type).lower() or uploaded_file_name.lower().endswith(".pdf"):
                        import fitz
                        doc = fitz.open(stream=target_new_sketch_bytes, filetype="pdf")
                        try:
                            vision_page_index = 0
                            for page_num in range(min(len(doc), 4)):
                                ...
                                if ("SKETCH" in page_text or "DRAWING" in page_text or "VISUAL" in page_text) and "TABLE OF CONTENTS" not in page_text:
                                    vision_page_index = page_num
                                    break
                            if vision_page_index == 0 and len(doc) > 1:
                                vision_page_index = 1
                                
                            page = doc[vision_page_index]
                            pix = page.get_pixmap(dpi=150)
                            gemini_ready_bytes = pix.tobytes("jpeg")
                            gemini_mime = "image/jpeg"
                        finally: doc.close()
                    else:
                        gemini_ready_bytes = target_new_sketch_bytes
                        gemini_mime = detected_mime_type

                    st.image(gemini_ready_bytes, caption=f"🖼️ [DEBUG] PAGE SENT TO GEMINI (Trang số {vision_page_index+1})", use_container_width=True)

                    if types and hasattr(types, "Part"):
                        ocr_contents = [types.Part.from_text(text=ocr_prompt), types.Part.from_bytes(data=gemini_ready_bytes, mime_type=gemini_mime)]
                    else:
                        ocr_contents = [ocr_prompt, {"mime_type": gemini_mime, "data": gemini_ready_bytes}]
                    ...
                    if ocr_res and ocr_res.text:
                        ...
                        st.text_area("📄 [DEBUG] RAW GEMINI OUTPUT", value=ocr_res.text, height=250)
                except Exception as gemini_err: st.error(f"🚨 Gemini Error: {gemini_err}")

            if len(new_vec) >= 10:
                type_match = re.search(r"GARMENT[\s_-]*TYPE\s*[:=]\s*([A-Z_\-]+)", new_vec, re.IGNORECASE)
                if type_match: st.session_state["detected_garment_type"] = type_match.group(1).strip().upper()
                else:
        ...
        with st.expander("🛠️ DEBUG: TRẠNG THÁI PHÂN LOẠI VISION CUỐI CÙNG", expanded=False):
        ...
        if len(new_vec) < 10:
            ...
            st.stop()
                ...
                # =========================================================================================
        # ĐOẠN 4 - PHẦN 2 - KHỐI C: TẢI DỮ LIỆU RẬP MẪU VÀ LỌC ỨNG VIÊN ĐỐI SOÁT
        # =========================================================================================
                ...
                if raw_styles and client and client.models:
                    ...
                    if target_new_sketch_bytes:
                        ...
                        if types and hasattr(types, "Part"):
                            ...
                            vision_contents.append(types.Part.from_bytes(data=gemini_ready_bytes, mime_type=gemini_mime))
                        else:
                            ...
                            vision_contents.append({"mime_type": gemini_mime, "data": gemini_ready_bytes})
                    ...
                    for idx, s in enumerate(top_8_candidates):
                        ...
                        historical_pool_summary.append({
                            "pool_index": idx, "style_name": s.get("StyleName"), "base_size": s.get("BaseSize", "N/A"), 
                            "features": str(s.get("sketch_vector", "")).strip()[:1000], "detailed_measurements": s.get("DetailedMeasurements", {})
                        })
                    ...
                    if json_match:
                        ...
                        if s_idx is not None and 0 <= s_idx < len(top_8_candidates) and score >= 65:
                            ...
                            st.session_state["matched_techpack"] = None
                            if score > 0 and score < 65: st.warning(f"⚠️ Điểm số đối soát đạt {score}% (Dưới ngưỡng an toàn 65%).")
                            else: st.error("🚨 Kết quả phân tích từ AI không khớp với bất kỳ mã hàng lịch sử nào.")
            except Exception as e: st.error(f"Lỗi hệ thống trong quá trình đối soát VLM: {e}")
...
if matched_techpack and new_specs and old_specs:
    ...
    is_new_upper = any(k in new_keys_str for k in ["CHEST", "BUST", "COLLAR", "PLACKET", "NECK", "ARMHOLE"])
    ...
    if (is_new_upper and is_old_lower) or (is_new_lower and is_old_upper):
        ...
        old_specs = {}
        st.error("🚨 **GARMENT LINE FIREWALL:** Phát hiện lỗi bắt cặp sai chủng loại (Áo đối soát với Quần). Hệ thống tự động chuyển sang chế độ Dự Phòng Hình Học độc lập!")
        st.rerun()
...
if new_specs or old_specs:
    ...
    # 🧵 PHÂN HỆ A: QUẦN / SHORT / JOGGER / CARGO (Thân dưới)
    if any(cat in category_context for cat in ["PANT", "SHORT", "TROUSER", "CARGO", "JOGGER"]):
        POM_WEIGHTS = {
            "HIP": 3.0, "THIGH": 2.5, "WAIST": 2.0, "WAIST_RELAX": 2.0, "WAIST_STRETCH": 2.0,
            "INSEAM": 2.5, "OUTSEAM": 2.5, "FRONT_RISE": 1.5, "BACK_RISE": 1.5, "KNEE": 1.0, "LEG_OPENING": 0.5
        }
        CORE_POMS = {"WAIST", "WAIST_RELAX", "WAIST_STRETCH", "HIP", "THIGH", "INSEAM", "OUTSEAM", "FRONT_RISE", "BACK_RISE", "KNEE", "LEG_OPENING"}
        BODY_ZONE_LOCK = {
            "WAIST_RELAX": "BOTTOM_WAIST_ZONE", "WAIST_STRETCH": "BOTTOM_WAIST_ZONE", "WAIST": "BOTTOM_WAIST_ZONE",
    ...
    # 🧵 PHÂN HỆ B: ÁO SƠ MI / T-SHIRT / POLO / HOODIE (Thân trên)
    elif any(cat in category_context for cat in ["SHIRT", "TEE", "POLO", "HOODIE", "TOP"]):
        POM_WEIGHTS = {
            "CHEST": 3.0, "BUST": 3.0, "BODY_LENGTH": 2.5, "SLEEVE_LENGTH": 2.0, 
            "ARMHOLE": 1.5, "SHOULDER": 1.5, "NECK": 1.0, "COLLAR": 1.0, "CUFF": 0.5
        }
        CORE_POMS = {"CHEST", "BUST", "BODY_LENGTH", "SLEEVE_LENGTH", "ARMHOLE", "SHOULDER", "NECK", "COLLAR", "CUFF"}
        BODY_ZONE_LOCK = {
            "CHEST": "TORSO_ZONE", "BUST": "TORSO_ZONE", "BODY_LENGTH": "LENGTH_ZONE", 
            "SLEEVE_LENGTH": "SLEEVE_ZONE", "ARMHOLE": "ARMHOLE_ZONE", "SHOULDER": "SHOULDER_ZONE",
            "NECK": "COLLAR_ZONE", "COLLAR": "COLLAR_ZONE", "CUFF": "SLEEVE_ZONE"
        }
        
    # 🧵 PHÂN HỆ C: ÁO JACKET / OUTERWEAR / COAT (Áo khoác tốn định mức vải)
    elif any(cat in category_context for cat in ["JACKET", "OUTERWEAR", "COAT", "BLAZER"]):
        POM_WEIGHTS = {
            "CHEST": 3.0, "BODY_LENGTH": 3.0, "SLEEVE_LENGTH": 2.5, "ARMHOLE": 2.0, 
            "SHOULDER": 1.5, "WAIST_JACKET": 1.5, "BOTTOM_HEM": 1.5, "PLACKET_LENGTH": 1.5, "HOOD_SIZE": 1.0
        }
        CORE_POMS = {"CHEST", "BODY_LENGTH", "SLEEVE_LENGTH", "ARMHOLE", "SHOULDER", "WAIST_JACKET", "BOTTOM_HEM", "PLACKET_LENGTH", "HOOD_SIZE"}
        BODY_ZONE_LOCK = {
            "CHEST": "TORSO_ZONE", "BODY_LENGTH": "LENGTH_ZONE", "SLEEVE_LENGTH": "SLEEVE_ZONE",
            "ARMHOLE": "ARMHOLE_ZONE", "SHOULDER": "SHOULDER_ZONE", "WAIST_JACKET": "TORSO_ZONE",
            "BOTTOM_HEM": "HEM_ZONE", "PLACKET_LENGTH": "PLACKET_ZONE", "HOOD_SIZE": "HOOD_ZONE"
        }
        
    # 🧵 PHÂN HỆ D: VÁY / CHÂN VÁY (DRESS / SKIRT)
    else:
        POM_WEIGHTS = {
            "TOTAL_LENGTH": 3.0, "BUST": 2.5, "WAIST": 2.5, "HIP": 2.5, 
            "SKIRT_SWEEP": 3.5, "BOTTOM_HEM": 1.5, "STRAP_LENGTH": 0.5
        }
        CORE_POMS = {"TOTAL_LENGTH", "BUST", "WAIST", "HIP", "SKIRT_SWEEP", "BOTTOM_HEM", "STRAP_LENGTH"}
        BODY_ZONE_LOCK = {
            "TOTAL_LENGTH": "LENGTH_ZONE", "BUST": "TORSO_ZONE", "WAIST": "DRESS_WAIST_ZONE",
            "HIP": "DRESS_HIP_ZONE", "SKIRT_SWEEP": "SWEEP_ZONE", "BOTTOM_HEM": "HEM_ZONE", "STRAP_LENGTH": "SHOULDER_ZONE"
        }
    ...
    POM_ALIAS = {
        ...
        "SLEEVE LENGTH FROM CB": "SLEEVE_LENGTH", "LONG SLEEVE": "SLEEVE_LENGTH", "SHOULDER WIDTH": "SHOULDER",
        "ACROSS SHOULDER": "SHOULDER", "COLLAR WIDTH": "COLLAR", "NECK WIDTH": "NECK", "ARMHOLE STRAIGHT": "ARMHOLE",
        "SWEEP WIDTH": "SKIRT_SWEEP", "BOTTOM SWEEP": "SKIRT_SWEEP", "SKIRT LENGTH": "TOTAL_LENGTH", "DRESS LENGTH": "TOTAL_LENGTH"
    }
    ...
    def calculate_advanced_cost(new_key, old_key, val_new, val_old):
        std_new = get_pom_standard(new_key)
        std_old = get_pom_standard(old_key)
        if std_new == std_old:
            return 0.0
        sm = SequenceMatcher(None, clean_pom_text(new_key), clean_pom_text(old_key))
        return 1.0 - sm.ratio()

    # --- Thêm phần logic so khớp chạy thực tế ---
    new_keys = list(new_specs.keys())
    old_keys = list(old_specs.keys())

    for nk in new_keys:
        v_new = clean_float(new_specs[nk])
        ...
        for ok in old_keys:
            v_old = clean_float(old_specs[ok])
            if v_old is None: continue
            
            cost = calculate_advanced_cost(nk, ok, v_new, v_old)
            if cost < min_cost:
                min_cost = cost
                best_match = ok
                
        if best_match and min_cost < 0.6:
            v_old = clean_float(old_specs[best_match])
            std_name = get_pom_standard(nk)
            weight = POM_WEIGHTS.get(std_name, 1.0)
            
            diff = v_new - v_old
            diff_pct = (diff / v_old * 100) if v_old != 0 else 0.0
            
            compare_rows.append({
                "POM": nk,
                "Matched Target": best_match,
                "New Size (cm)": v_new,
                "Techpack Size (cm)": v_old,
                "Difference": diff,
                "Diff (%)": diff_pct,
                "Zone": BODY_ZONE_LOCK.get(std_name, "OTHER_ZONE"),
                "Weight": weight
            })
            weighted_impact_sum += abs(diff_pct) * weight
            total_impact_weight += weight

    if total_impact_weight > 0:
        avg_pom_diff_pct = weighted_impact_sum / total_impact_weight
        st.metric(label="📊 Chỉ số sai lệch trọng số Rập (Weighted Deviation Index)", value=f"{avg_pom_diff_pct:.2f}%")
        st.dataframe(compare_rows)
    else:
        st.warning("⚠️ Không tìm thấy thông số kỹ thuật tương thích nào để đối chiếu.")
