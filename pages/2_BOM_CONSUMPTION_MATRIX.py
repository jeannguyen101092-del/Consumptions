...
def ai_consumption_analyst_engine(client, user_message, matched_techpack, bom_records, new_style_measurements, target_new_sketch_bytes, detected_size):
    ...
    ĐÃ TỐI ƯU: Ép AI trả về kết quả số thẳng thừng, tuyệt đối không giải thích dài dòng.
    """
    ...
    if matched_techpack:
        scenario_instruction = f"KỊCH BẢN ĐỒNG DẠNG: So sánh Spec mới và Spec cũ {json.dumps(specs_old)} dựa trên BOM gốc {bom_summary}."
    else:
        scenario_instruction = f"KỊCH BẢN VECTOR HÌNH HỌC: Tự tính diện tích thô dựa trên New Spec: {json.dumps(new_style_measurements)} và khổ vải {f_width if f_width > 0 else '58 inch'}."

    system_instruction = f"""
    You are an expert Garment Costing Engineer at PPJ Group.
    CRITICAL CONSTRAINT: Provide ONLY the final numeric calculation answers. DO NOT explain the math, DO NOT provide step-by-step logic, and DO NOT output conversational sentences.
    
    STRICT FORMAT REQUIREMENT:
    - First line: Output EXACTLY the final consumption value in YARDS (Yds). Example: "Định mức trung bình: 1.45 Yds"
    - Sub-lines (Maximum 2 lines): Only list raw final parts consumption numbers if needed. NO detailed explanation text.
    
    RULES:
    - Seam allowance: 0.44 inches.
    - Hem / Placket rules applied based on category.
    {scenario_instruction}
    - Size: Size {detected_size}
    """
    ...
    try:
        ...
        ai_reply = response.text.strip() if response.text else "Hệ thống AI không thể đưa ra phân tích."
...
def process_single_pdf_batch(file_bytes, file_name):
    """
    Hàm bóc tách dữ liệu kỹ thuật từ một file PDF độc lập.
    ĐÃ TỐI ƯU CACHE STATE: Tránh việc AI phải quét lại hồ sơ liên tục gây tốn dung lượng băng thông.
    """
    import time
    # Tạo khóa cache độc nhất dựa vào tên file và dung lượng dữ liệu
    cache_key = f"pdf_cache_{hash(file_name)}_{len(file_bytes)}"
        ...
        for attempt in range(3):
            try:
                ...
                if response and response.text:
                    ...
                    # Lưu trữ vào bộ nhớ đệm cache state để tái sử dụng ngay lập tức
                    st.session_state[cache_key] = result_data
                    return result_data
            except Exception:
                time.sleep(1.5)
                continue
...
# Khởi tạo bộ nhớ đệm phòng vệ nếu chưa có trong trạng thái phiên chạy
...
# Hợp nhất dữ liệu từ các nguồn tải tệp lên giao diện
...
# CƠ CHẾ ĐƯỜNG THẲNG: Chỉ chạy quét AI duy nhất 1 lần khi có file mới
if has_file:
    file_bytes = target_file_object.getvalue()
    file_name = target_file_object.name
    
    if st.session_state["cached_extracted_data"] is None or st.session_state.get("last_processed_file") != file_name:
        if file_name.lower().endswith('.pdf'):
            res_pdf = process_single_pdf_batch(file_bytes, file_name)
            if res_pdf.get("success"):
                st.session_state["cached_extracted_data"] = res_pdf
                st.session_state["last_processed_file"] = file_name
        else:
            st.session_state["cached_extracted_data"] = {"success": True, "data": {}, "sketch_bytes": file_bytes}
            st.session_state["last_processed_file"] = file_name

    # Đổ dữ liệu từ Cache State ra biến cấu trúc nền tảng của hệ thống
    if st.session_state["cached_extracted_data"]:
        c_res = st.session_state["cached_extracted_data"]
        meta_p = c_res.get("data", {})
        ...
        target_new_sketch_bytes = c_res.get("sketch_bytes")
...
if menu_selection == "🧵 BOM & Consumption Matrix":
    ...
    # === THANH TÌM KIẾM THỦ CÔNG (ĐẶT TRÊN LỆNH CHẶN FILE ĐỂ HOẠT ĐỘNG TRIỆT ĐỂ) ===
        ...
        if st.button(f"⚡ Trích xuất nhanh dữ liệu mã hàng: {search_query.strip().upper()}", use_container_width=True):
            with st.spinner("🔍 Đang định vị hồ sơ cục bộ..."):
                try:
                    ...
                    # Giữ nguyên 100% toán tử ilike cũ của bạn
                    query_params_search = {"select": "StyleName,Buyer,Category,BaseSize,DetailedMeasurements,SketchURL,sketch_vector", "StyleName": f"ilike.*{search_query.strip()}*", "limit": 1}
                    s_res = requests.get(url_search, headers=headers, params=query_params_search, timeout=10)
                    
                    if s_res.status_code == 200 and s_res.json():
                        raw_list = s_res.json()
                        # Lấy phần tử đầu tiên nếu kết quả trả về là một mảng danh sách
                        st.session_state["matched_techpack"] = raw_list[0] if isinstance(raw_list, list) else raw_list
    ...
    # === LỆNH CHẶN FILE: CHUYỂN XUỐNG ĐÂY ĐỂ THANH TÌM KIẾM THỦ CÔNG KHÔNG BỊ BIẾN MẤT ===
    if not has_file:
    ...
    # Chỉ chạy quét AI tự động nếu kỹ sư chưa chọn mã thủ công qua thanh công cụ
                ...
                if all_historical_styles:
                    ...
                    for idx, s in enumerate(all_historical_styles):
                        styles_pool_summary.append({
                            "pool_index": idx, "style_name": s.get("StyleName"), "sketch_features_vector": s.get("sketch_vector", "")
                        })

                    match_prompt = f"Select the single index that represents the exact match. HISTORICAL POOL DATA: {json.dumps(styles_pool_summary)} Return JSON: {{\"selected_pool_index\": 0}}"
