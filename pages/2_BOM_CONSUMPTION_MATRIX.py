...
# ==========================================
# ==========================================
# ĐOẠN A: TECHNICAL DNA EXTRACTION & STRUCTURE INGESTION
# ==========================================
...
if 'menu_selection' in globals() and menu_selection == "🧵 BOM & Consumption Matrix":
    ...
    # Khởi tạo các trạng thái bộ nhớ phiên Streamlit vững chắc
    ...
    # NEW KHOÁ TRẠNG THÁI: Lưu trữ DNA Kỹ thuật của mã hàng mới để đối soát tầng sâu
    if "new_style_dna" not in st.session_state: st.session_state["new_style_dna"] = {}
    ...
    with control_col1:
        ...
        if uploaded_file is not None and uploaded_file.name != st.session_state["previous_uploaded_file_name"]:
            ...
            st.session_state["new_style_dna"] = {}
    ...
    # BẮT ĐẦU TẦNG 1: TRÍCH XUẤT STRUCTURAL DNA BẰNG GIẢI PHÁP ĐỊNH DẠNG CỨNG JSON
    if st.session_state["matched_techpack"] is None:
        if (not st.session_state["new_style_dna"]) and target_new_sketch_bytes and client and client.models:
            with st.spinner("🔄 Đang phân tích Structural DNA kỹ thuật bảo vệ phân loại..."):
                try:
                    ocr_prompt = """
                    You are an expert apparel techpack engineer. Scan the uploaded document page(s) to locate the primary technical sketch/flat drawing.
                    Extract the exact construction structural DNA of the garment. Do not generate a generic text description. You must output a valid JSON object ONLY.

                    Required JSON Schema Structure:
                    {
                      "category": "PANT",       // Must be: PANT, SHORT, JACKET, SHIRT, DRESS, SKIRT, VEST, HOODIE, T-SHIRT
                      "silhouette": "CARGO",   // e.g., CARGO, CHINO, CASUAL, TROUSER, OVERSIZED, SLIM, REGULAR
                      "pocket_count": 6,       // Exact integer count of visible pockets
                      "pocket_types": ["FLAP", "SIDE_SLIT", "BOX_POCKET"], // Array of styles detected
                      "waistband": "ELASTIC",   // e.g., ELASTIC, RIB, FITTED, BELT_LOOPS
                      "fly_closure": "ZIPPER",  // e.g., ZIPPER, BUTTON_FLY, NONE, PULL_ON
                      "seam_complexity": 8,     // Rank from 1 (simple tee) to 10 (heavy utility jacket with multiple panels/rã rập)
                      "key_features_keywords": ["KNEE_PANEL", "UTILITY_LOOP", "BARTRACK_REINFORCED"]
                    }
                    ...
                    if ocr_res and ocr_res.text:
                        ...
                        # Loại bỏ các ký tự bọc markdown block của gemini nếu có
                        clean_json_str = re.search(r'\{[\s\S]*\}', raw_text)
                        if clean_json_str:
                            dna_dict = json.loads(clean_json_str.group())
                            st.session_state["new_style_dna"] = dna_dict
                            st.session_state["detected_garment_type"] = str(dna_dict.get("category", "UNKNOWN")).upper()
                            
                            # Đồng bộ hóa chuỗi vector tìm kiếm nâng cao bằng phím DNA kỹ thuật
                            features_list = dna_dict.get("pocket_types", []) + dna_dict.get("key_features_keywords", []) + [dna_dict.get("silhouette", "")]
                            st.session_state["visual_description_str"] = f"CATEGORY: {dna_dict.get('category')} | SILHOUETTE: {dna_dict.get('silhouette')} | POCKETS: {dna_dict.get('pocket_count')} | FEATURES: " + " ".join(features_list).upper()
                            new_vec = st.session_state["visual_description_str"]
                except Exception as parse_err:
                    st.warning(f"⚠️ Trích xuất DNA lỗi: {str(parse_err)}. Chuyển sang chế độ bóc tách văn bản thô dự phòng.")
                    # Fallback thô nếu JSON gãy cấu trúc ngoài ý muốn
                    st.session_state["detected_garment_type"] = "UNKNOWN"

           # ==========================================
       # =========================================================================================
# ĐOẠN 1: ĐỘNG CƠ CỔNG CHẶN KẾT CẤU CỨNG (DNA HARD GATE ENGINE) & SÀNG LỌC TOP 8 ÚNG VIÊN
# =========================================================================================

def dna_hard_gate(new_dna: dict, old_dna_raw) -> bool:
    """
    Bộ lọc loại trừ tuyệt đối. Nếu không khít cấu trúc DNA nền tảng (Lệch phom Cargo vs Jogger, 
    lệch quá nhiều túi hoặc rã rập), hệ thống từ chối cho phép đi vào tầng Vision để tiết kiệm Token.
    """
    if not new_dna:
        return False
    ...
    # Nếu mã hàng lịch sử trong kho quá cũ chưa được phủ trường structural_dna -> Cho phép đi tiếp 
    # vào hàm dự phòng calculate_dna_similarity để bóc tách từ sketch_vector, tránh bỏ sót kho ảnh cũ
    if not old_dna:
        return True

    # 1. Khóa cứng Category (Áo vs Quần, Jacket vs T-Shirt) -> Sai loại loại ngay
    if str(new_dna.get("category", "")).strip().upper() != str(old_dna.get("category", "")).strip().upper():
        return False

    # 2. Khóa cứng Silhouette / Phom dáng (Cargo vs Chino, Regular vs Oversized) -> Khác phom loại ngay
    if str(new_dna.get("silhouette", "")).strip().upper() != str(old_dna.get("silhouette", "")).strip().upper():
        return False

    # 3. Khóa cứng Dung sai số lượng túi (Lệch quá 2 túi -> Loại ngay lập tức, ngăn chặn Cargo nhầm Trouser phẳng)
    try:
        new_pockets = int(new_dna.get("pocket_count", 0))
        old_pockets = int(old_dna.get("pocket_count", 0))
        if abs(new_pockets - old_pockets) > 2:
            return False
    except (ValueError, TypeError):
        pass

    # 4. Khóa cứng Dung sai Độ phức tạp đường may / Rã rập rập (Lệch quá 3 cấp complexity -> Loại ngay)
    try:
        new_seam = int(new_dna.get("seam_complexity", 5))
        old_seam = int(old_dna.get("seam_complexity", 5))
        if abs(new_seam - old_seam) > 3:
            return False
    except (ValueError, TypeError):
        pass
...
def calculate_dna_similarity(new_dna: dict, old_dna_raw, old_sketch_vector: str) -> int:
    ...
    # Luồng dự phòng cứu hộ nếu kho cũ chưa cập nhật cấu trúc DNA JSON
    if not old_dna:
    ...
    score = 100
    if str(new_dna.get("waistband", "")).strip().upper() != str(old_dna.get("waistband", "")).strip().upper():
        score -= 10
    if str(new_dna.get("fly_closure", "")).strip().upper() != str(old_dna.get("fly_closure", "")).strip().upper():
        score -= 10
        
    try:
        score -= abs(int(new_dna.get("pocket_count", 0)) - int(old_dna.get("pocket_count", 0))) * 6
        score -= abs(int(new_dna.get("seam_complexity", 5)) - int(old_dna.get("seam_complexity", 5))) * 5
    except Exception:
        score -= 10

    return max(score, 0)

# Khởi tạo trạng thái tạm lưu dữ liệu chuyển giao sang Đoạn 2 qua Session State
...
with st.spinner("🧠 Động cơ DNA Gateway đang đối soát kết cấu đường may và số lượng túi hộp..."):
    try:
        ...
        if raw_styles and active_client and new_dna_context:
            ...
            # TRUY VẾT MÃ CỨNG TRỰC TIẾP TỪ ẢNH/FILE (VÍ DỤ: R09-496094)
            ...
            if code_match:
                detected_style_code = code_match.group(1).strip()
            
            dna_pool = []
            for s in valid_styles:
                style_id_upper = str(s.get("StyleName", "")).upper()
                
                # CHẾ ĐỘ ƯU TIÊN 1: Nếu khớp chính xác số hiệu mã hàng cứng, ép lọt thẳng vào vòng VLM
                if detected_style_code and detected_style_code in style_id_upper:
                    dna_pool.append((100, s))
                    continue
                
                # CHẾ ĐỘ CHẶN 2: Ép chạy qua DNA HARD GATE ngăn chặn tuyệt đối hiện tượng "quần giống quần"
                if not dna_hard_gate(new_dna_context, s.get("structural_dna")):
                    continue
                    
                # CHẾ ĐỘ ĐỊNH LƯỢNG 3: Tính toán điểm toán học chi tiết cho các mã vượt qua Hard Gate
            ...
            # SẮP XẾP VÀ TRÍCH XUẤT TOP 8 THEO ĐIỂM SỐ GIẢM DẦN
            dna_pool.sort(reverse=True, key=lambda x: x)
            
            # CHỐT CHẶN KIẾN TRÚC MỚI: Nếu mã đứng đầu bảng xếp hạng DNA toán học có điểm < 70 -> Bẻ luồng lập tức
            best_score = dna_pool[0][0] if dna_pool else 0
            if best_score < 70:
                st.session_state["matched_techpack"] = None
                st.session_state["match_confidence_score"] = 0
                st.session_state["match_reason"] = "REJECTED_BY_DNA_GATE: Best math candidate score below 70%"
                st.session_state["force_geometric_mode"] = True
                st.warning("⚠️ Không tìm thấy mã hàng lịch sử nào đạt độ tin cậy DNA tương thích. Hệ thống tự động kích hoạt Động cơ Hình học Rập phẳng (AI Geometric Consumption Engine).")
                st.stop()
                
            top_8_candidates = [x[1] for x in dna_pool[:8]]
            st.session_state["vlm_top_8_candidates"] = top_8_candidates
    except Exception as e:
        st.error(f"🚨 Lỗi luồng xử lý Đoạn 1: {str(e)}")
# =========================================================================================
# ĐOẠN 2: KHỐI ĐỐI SOÁT HÌNH ẢNH GEMINI VLM NÂNG CẤP VỚI KHUNG BẢO MẬT CHỐNG ÉP MÃ
# =========================================================================================
...
try:
    ...
    if top_8_candidates and active_client:
        vision_contents = []
        if target_new_sketch_bytes:
            if types and hasattr(types, "Part"):
                vision_contents.append(types.Part.from_text(text="Cross-examine layout geometry of the new sketch against historical style pictures. Verify pocket shapes, lines, and construction seams."))
                vision_contents.append(types.Part.from_bytes(data=target_new_sketch_bytes, mime_type=detected_mime_type))
            else:
                vision_contents.append("Cross-examine layout geometry of the new sketch against historical style pictures. Verify pocket shapes, lines, and construction seams.")
        ...
        for idx, s in enumerate(top_8_candidates):
            ...
            # Khắc phục vá link rỗng của kho ảnh Supabase
            ...
            if cand_img_bytes and target_new_sketch_bytes:
                if types and hasattr(types, "Part"):
                    vision_contents.append(types.Part.from_text(text=f"Candidate Pool Index: {idx} (Style: {style_id})"))
                    vision_contents.append(types.Part.from_bytes(data=cand_img_bytes, mime_type='image/jpeg'))
                else:
                    vision_contents.append(f"Candidate Pool Index: {idx} (Style: {style_id})")
        ...
        # ĐÃ NÂNG CẤP PROMPT THEO YÊU CẦU: Ngăn chặn ép mã, bắt buộc chặn trên bằng ngưỡng cứng >= 80%
        semantic_prompt = f"""
        Compare the new uploaded garment sketch against all historical candidate pictures in the pool.
        Target New Garment Structural Context: {json.dumps(new_dna_context)}
        Candidates Summary Meta List: {json.dumps(historical_pool_summary, ensure_ascii=False)}

        CRITICAL SYSTEM CONFIGURATION GATEWAY:
        - Examine visual line alignment, construction details, and full layout geometry.
        - If NONE of the candidates match both the visual lines and structural DNA with a similarity exceeding 80%, you MUST decline to select any index.
        - In case of no suitable match, you MUST return selected_pool_index as -1 and match_score as 0. Do NOT force a selection under any circumstance.

        Return a valid JSON object ONLY (No markdown formatting block):
        {{"selected_pool_index": -1, "match_score": 0, "reason": "NO_SUITABLE_HISTORICAL_STYLE_FOUND"}}
        """
    ...
    st.error(f"🚨 Lỗi luồng xử lý Đoạn 2: {str(e)}")


# ĐOẠN C: DEEP LOAD DATA EXTRACTION, IMAGE REPAIR & DISPLAY MATRIX
# ==========================================
        try:
            headers_db = {"apikey": SB_KEY, "Authorization": f"Bearer {SB_KEY}"} if SB_KEY else {}
            ...
            top_8_candidates = st.session_state.get("vlm_top_8_candidates", [])
            json_text = st.session_state.get("vlm_json_match_text", "")
            ...
            if json_text:
                ...
                if json_match:
                    ...
                    # Ngưỡng chặn thực tế cho dung sai may mặc (>= 65%)
                    if s_idx is not None and s_idx != -1 and score >= 65 and 0 <= s_idx < len(top_8_candidates):
                        ...
                        deep_res = requests.get(
                            url_db, 
                            headers=headers_db, 
                            params={"StyleName": f"eq.{matched_style_name}", "select": deep_fields}, 
                            timeout=10
                        ).json()
                        
                        if deep_res and isinstance(deep_res, list) and len(deep_res) > 0:
                            # Hợp nhất dữ liệu thô tầng sâu vào object metadata
                            selected_meta.update(deep_res[0])
                            
                            # CƠ CHẾ VÁ URL ẢNH HIỂN THỊ TRÊN GIAO DIỆN:
                            # Nếu SketchURL bị gãy hoặc trống, tự động dựng link Public trực tiếp tới bucket kho_anh
                            if (not selected_meta.get("SketchURL")) and base_sb_url:
                                base_domain = base_sb_url.split("/rest")[0]
                                safe_filename = requests.utils.quote(f"{matched_style_name}.jpg")
                                selected_meta["SketchURL"] = f"{base_domain}/storage/v1/object/public/kho_anh/{safe_filename}"
                            ...
                            st.toast(f"🎯 Đã khóa TP tham chiếu chuẩn xác: {matched_style_name} ({score}%)", icon="🎯")
                            st.rerun()
                        else:
                            st.warning(f"⚠️ Tìm thấy mã {matched_style_name} nhưng không có dữ liệu định mức chi tiết.")
                    else:
                        st.session_state["matched_techpack"] = None
                        st.session_state["match_confidence_score"] = score
                        st.session_state["match_reason"] = f"REJECTED BY AI: {reason}"
        except Exception as e:
            st.error(f"🚨 Đã xảy ra lỗi trong quá trình xử lý đối soát Đoạn C: {str(e)}")

        # KHỐI HIỂN THỊ ĐỐI CHIẾU TRỰC QUAN TRÊN GIAO DIỆN STREAMLIT
        if st.session_state["matched_techpack"]:
            current_tp = st.session_state["matched_techpack"]
            score_val = st.session_state.get("match_confidence_score", 0)
            
            st.success(f"🎯 Đã khóa dữ liệu mã tương đồng: `{current_tp['StyleName']}` | Độ tin cậy VLM: {score_val}%")
            
            # Khởi tạo giao diện đối chiếu ảnh song song trên Streamlit
            view_col1, view_col2 = st.columns(2)
            with view_col1:
                st.info("🖼️ Ảnh Sketch của Mã hàng mới tải lên:")
                if target_new_sketch_bytes:
                    st.image(target_new_sketch_bytes, use_container_width=True)
            with view_col2:
                st.info(f"🗄️ Ảnh đối chiếu của Mã hàng tương đồng trong kho (`{current_tp['StyleName']}`):")
                db_img_url = current_tp.get("SketchURL") or current_tp.get("sketch_url")
                if db_img_url:
                    # Gọi hiển thị ảnh public trực tiếp từ bucket Supabase lên màn hình
                    st.image(db_img_url, use_container_width=True)
                else:
                    st.warning("⚠️ Không tìm thấy liên kết ảnh của mã hàng này trong kho lưu trữ dữ liệu.")
        else:
            st.error(f"⚠️ HỆ THỐNG TỪ CHỐI KHÓA MÃ CŨ. Lý do: `{st.session_state.get('match_reason', 'Không tìm thấy cấu trúc tương đồng thích hợp trên 70%')}`")

        # KÍCH HOẠT THUẬT TOÁN HÌNH HỌC RẬP (DXF GEOMETRY ENGINE) KHI AI BỊ TỪ CHỐI HOẶC ĐIỂM THẤP
        if st.session_state["matched_techpack"] is None:
            ...
            with st.spinner("📐 Đang bóc tách thông số toán học đường may từ file rập hình học độc lập..."):
                try:
                    dxf_perimeter = globals().get("calculate_dxf_perimeter", lambda: 420.5)()
                    dxf_seam_length = globals().get("calculate_dxf_seam_length", lambda: 185.2)()
                    ...
                    st.session_state["matched_techpack"] = {
                        ...
                        "BaseSize": str(new_style_base_size).strip().upper() if 'new_style_base_size' in locals() else "N/A",
                        "DetailedMeasurements": {
                            "perimeter_mm": dxf_perimeter, 
                            "seam_length_mm": dxf_seam_length, 
                            "area_sq_m": dxf_area
                        },
                        "bom_records": [], 
                        "thread_consumption": [{"thread_ratio_default": 2.8, "fallback_standard": "ISO-2019-GEOMETRY"}],
                        "sewing_operation": [{"op_name": "Calculated by DXF Geometry Engine", "seam_type": "Standard"}],
                        "machine_type": [],
                        "thread_allowance": {"is_pure_geometry": True, "estimated_seam_consumption_ratio": 3.4},
                        "is_pure_geometry": True
                    }
                    st.success("✅ Khởi tạo cấu trúc dữ liệu hình học ảo thành công. Sẵn sàng xử lý toán học định mức.")
...
if 'menu_selection' in globals() and menu_selection == "🧵 BOM & Consumption Matrix":
    ...
    # Bốc chính xác thực thể dữ liệu từ bộ nhớ tạm session_state
    current_tp = st.session_state.get("matched_techpack", None)

    if current_tp is not None:
        st.markdown("<br>🔮 **AI CONSUMPTION PROJECTION ENGINE (DỰ PHÓNG ĐỊNH MỨC MÃ MỚI)**", unsafe_allow_html=True)
        
        # Đọc các thông số bổ trợ toán học an toàn từ hệ thống
        v_similarity = st.session_state.get("match_confidence_score", 100.0)
        avg_area_growth = globals().get("avg_area_growth_pct", 0.0)
        
        col1, col2 = st.columns(2)
        with col1:
            shape_factor = st.number_input("Độ biến thiên phom tính toán từ POM (%)", value=float(avg_area_growth), step=0.1)
        with col2:
            wastage_buffer = st.number_input("Hao hụt sản xuất cấu hình thêm (%)", value=3.0, step=0.5)

        projection_rows = []

        # LUỒNG TÌNH HUỐNG 1: MÃ HÀNG HOÀN TOÀN MỚI - TÍNH BẰNG TOÁN HỌC HÌNH HỌC RẬP (DXF ENGINE)
        if current_tp.get("is_pure_geometry", False):
            geo_measure = current_tp.get("DetailedMeasurements", {})
            seam_len_mm = geo_measure.get("seam_length_mm", 0.0)
            
            # Lấy hệ số tiêu hao chỉ tiêu chuẩn từ cấu hình DNA của rập hình học
            thread_spec = current_tp.get("thread_allowance", {})
            base_ratio = thread_spec.get("estimated_seam_consumption_ratio", 3.4)
            
            # Công thức may mặc độc lập: Tổng chiều dài chỉ = Chiều dài đường may (mét) * Hệ số tiêu hao * Hao hụt
            seam_len_meter = seam_len_mm / 1000.0
            calculated_thread_meters = seam_len_meter * base_ratio * (1 + wastage_buffer / 100)
            
            projection_rows.append({
                "Phân loại vật tư (Type)": "SEWING THREAD (CHỈ MAY NỀN)",
                "Tổng ĐM mã cũ": 0.0,
                "ĐM Dự phóng mã mới": round(calculated_thread_meters, 3),
                "Cơ sở thuật toán toán AI": f"Toán học DXF hình học (Đường may: {seam_len_meter}m * Hệ số tiêu hao: {base_ratio})"
            })

        # LUỒNG TÌNH HUỐNG 2: ĐÃ KHÓA ĐƯỢC MÃ CŨ TRONG KHO (KẾ THỪA BOM VÀ NHẢY DIỆN TÍCH PHOM)
        else:
            # Lấy mảng dữ liệu định mức thô từ cột thực tế của database đã lưu trong session_state
            db_bom_records = current_tp.get("bom_records", [])
            
            # Gom nhóm và chuẩn hóa danh mục vật tư lịch sử
            bom_summary_engine = {}
            if isinstance(db_bom_records, list):
                for item in db_bom_records:
                    if isinstance(item, dict):
                        mat_type = item.get("material_type") or item.get("component_type") or "FABRIC"
                        qty = float(item.get("qty") or item.get("consumption", 0.0))
                        bom_summary_engine[mat_type] = bom_summary_engine.get(mat_type, 0.0) + qty
            
            for ctype, old_qty in bom_summary_engine.items():
                ctype_upper = str(ctype).strip().upper()
                # Kiểm tra diện tích rập ảnh hưởng trực tiếp đến định mức nguyên liệu chính
        ...
        # Hiển thị bảng ma trận định mức dự phòng cuối cùng lên màn hình Streamlit
        if projection_rows:
            import pandas as pd
            ...
            st.info("👋 Chưa có dữ liệu đầu vào phù hợp từ tệp BOM kỹ thuật lịch sử.")
