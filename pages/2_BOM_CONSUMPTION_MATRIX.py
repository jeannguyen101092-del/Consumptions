...
def ai_consumption_analyst_engine(client, user_message, matched_techpack, bom_records, new_style_measurements, target_new_sketch_bytes, detected_size):
    ...
    1. IF THE CATEGORY IS PANT / SHORT / SKORT / TROUSER (NHÓM HÀNG QUẦN):
       ...
       - Zipper Fly / Fly Placket (Cửa quần): Only calculate based on standard front fly extensions. Add this separate piece to the overall layout marker area.
...
def process_file_upload_logic(meta_data_dict):
    """Hàm tự động bắt đối tượng file tải lên và phân tách luồng dữ liệu byte dữ liệu kỹ thuật"""
        ...
        if file_name.lower().endswith('.pdf'):
            try:
                ...
                if res_pdf.get("success"):
                    meta_p = res_pdf["data"]
                    meta_data_dict["id"] = meta_p.get("style_number_parsed", "UNKNOWN_STYLE")
                    meta_data_dict["category"] = meta_p.get("category", "")
                    meta_data_dict["base_size"] = meta_p.get("base_size_name", "32")
                    meta_data_dict["measurements"] = meta_p.get("measurements", {})
                    meta_data_dict["sketch_bytes"] = res_pdf.get("sketch_bytes")
            except Exception:
                pass
        else:
            meta_data_dict["sketch_bytes"] = file_bytes
            
    return has_file, meta_data_dict
def run_gemini_vision_matching(base_sb_url, headers_db, target_new_sketch_bytes, client, types):
    """Hệ thống quét dữ liệu và so sánh phom dáng thiết kế qua Computer Vision Ingestion"""
    try:
        ...
        if all_historical_styles:
            ...
            1. WAISTBAND CONSTRUCTION (CẠP QUẦN): Look closely at the waistband stitches. 
               - If the new image shows a full elastic waistband (cạp chun co giãn toàn bộ, nhiều đường nhăn song song, NO belt loops, NO zipper fly), you MUST REJECT any historical styles that have belt loops, front zipper fly, or standard button closures.
...
def query_bom_records(base_sb_url, headers):
    """Hàm đối soát và tải thông tin chi tiết vật tư nguyên vật liệu (BOM) từ database"""
    if not st.session_state.get("matched_techpack"):
        return
    ...
    try:
        ...
        st.session_state["bom_records"] = []
def render_flat_sketch_comparison(target_new_sketch_bytes, new_style_id_detected, matched_techpack, base_sb_url, headers):
    """Hàm kiểm tra mã hóa nhị phân vật lý và hiển thị trực quan hóa 2 hình thiết kế đối soánh"""
    ...
    with img_col2:
        if matched_techpack:
            # Sửa đổi quan trọng: Gọi mã tương đồng và liên kết chuẩn xác với kho lưu trữ ảnh của bạn qua urllib.parse.quote
            ...
            encoded_style_name = quote(target_style_name)
            encoded_style_lower = quote(target_style_name.lower())
            
            url_options = [
                f"{base_sb_url}/storage/v1/object/public/kho_anh/{encoded_style_name}.jpg",
                f"{base_sb_url}/storage/v1/object/public/kho_anh/{encoded_style_name}.JPG",
                f"{base_sb_url}/storage/v1/object/public/kho_anh/{encoded_style_name}.jpeg",
                f"{base_sb_url}/storage/v1/object/public/kho_anh/{encoded_style_lower}.jpg"
            ]
...
def render_measurement_tables(new_style_measurements_dict, new_style_base_size, matched_techpack):
    """Hàm kết xuất cấu trúc Dataframe hiển thị song song hai bảng kích thước hình học rập mẫu"""
...
def render_historical_bom_table(bom_records):
    """Hàm hiển thị bảng định mức lịch sử"""
    if st.session_state.get("matched_techpack") and bom_records:
...
def handle_chatbot_engine(matched_techpack, bom_records, new_style_measurements_dict, target_new_sketch_bytes, new_style_base_size, client):
    """Vận hành hộp thoại chat box thông minh và ép trình duyệt tự động scroll lướt màn hình mượt mà xuống dưới"""
    ...
    if user_query := st.chat_input("Nhập yêu cầu phân tích (Ví dụ: Tính định mức vải chính khi co rút ngang 5%, dọc 3%)..."):
        ...
        # Đồng bộ lưu vết mảng dữ liệu chat box
        ...
        st.rerun()
# ==========================================
# KHỞI CHẠY KHỐI LỆNH TRÍCH XUẤT SẢN PHẨM MỚI
# ==========================================
meta_payload = {
    "id": "UNKNOWN_STYLE",
    "category": "",
    "fabric": "UNKNOWN_FABRIC",
    "measurements": {},
    "base_size": "32",
    "sketch_bytes": None
}

# Tiến hành bóc tách tệp tải lên qua hàm độc lập
has_file, meta_payload = process_file_upload_logic(meta_payload)

# Phân bổ lại các biến lõi hệ thống toàn cục
new_style_id_detected = meta_payload["id"]
new_style_category_detected = meta_payload["category"]
new_style_fabric_detected = meta_payload["fabric"]
new_style_measurements_dict = meta_payload["measurements"]
new_style_base_size = meta_payload["base_size"]
target_new_sketch_bytes = meta_payload["sketch_bytes"]

dynamic_keyword = str(new_style_id_detected).strip().upper()
base_sb_url = SB_URL.rstrip('/')
headers = {"apikey": SB_KEY, "Authorization": f"Bearer {SB_KEY}"}

# Bắt đầu luồng kiểm soát giao diện Menu BOM & Matrix
if menu_selection == "🧵 BOM & Consumption Matrix":
    ...
    with st.spinner("🧠 Hệ thống thị giác máy tính đang quét kết cấu phom dáng Flat Sketch..."):
        # Chạy hàm đối soát hình dáng qua Gemini
        run_gemini_vision_matching(base_sb_url, headers, target_new_sketch_bytes, client, types)

    # Chạy hàm tải bảng chi tiết định mức BOM
    query_bom_records(base_sb_url, headers)

    matched_techpack = st.session_state.get("matched_techpack")
    bom_records = st.session_state.get("bom_records", [])

    # Gọi cụm hàm kết xuất giao diện đồ họa và so khớp thông số POM
    render_flat_sketch_comparison(target_new_sketch_bytes, new_style_id_detected, matched_techpack, base_sb_url, headers)
    render_measurement_tables(new_style_measurements_dict, new_style_base_size, matched_techpack)
    render_historical_bom_table(bom_records)

    st.markdown("<br><hr style='border:0.5px solid #CBD5E1;'>", unsafe_allow_html=True)
    
    # Thiết lập thanh tiêu đề điều khiển Chatbox
